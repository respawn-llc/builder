...
- **No guarantee maximalism or non-functional-requirement gold-plating.** Match consistency, atomicity, durability, ordering, rollback, retry, and recovery guarantees to explicitly approved product risk and authoritative domain invariants.
  - Existing code, tests, locks, transactions, ownership boundaries, and recovery behavior are evidence to audit, not invariants to preserve. Remove or replace them when they conflict with specs, User decisions, or a simpler correct domain model.
  - This rule never authorizes unsafe concurrency, data races, swallowed errors, bypassed validation, or reporting success when the approved product operation did not complete.
  ...
  - Before adding a cross-cutting coordination mechanism—such as a long or multi-resource transaction, global mutex or permit, revision or epoch, retry/recovery state machine, compensation protocol, writer-path hook, or cross-authority snapshot—present the exact user-visible failure prevented, severity and data-loss potential, measured or estimated incidence, affected users, simpler weaker guarantee, and implementation blast radius. Obtain explicit User approval before implementation.
  - Identify the authoritative product owner and source of truth before choosing a coordination mechanism. Prefer enforcement inside that confirmed owner. Crossing ownership boundaries requires evidence that the owner cannot satisfy the approved product behavior alone.
  ...
  - Tests, review findings, and implementation convenience do not authorize stronger product guarantees. If a spec appears to require disproportionate machinery, stop and ask the User to confirm or weaken the guarantee.
...
- Full transcript history is unbounded & weighs dozens of gigabytes, thus **no production path may traverse `events.jsonl` from byte 0 to EOF** — even a bounded-memory full walk of the file is forbidden for transcript/working-set reads. Any requests from the user that result in such reads must be flagged with a question. Do not reintroduce in-memory full-transcript readers, a resident transcript buffer, or absolute `Offset`/`TotalEntries` pagination that needs a cumulative count from disk. Exemption (user-ratified): session fork or clone materialization may walk from byte 0 through the selected fork point because copying that history is the operation's explicit purpose; this does not authorize transcript reads, counts, or resident buffers. Exemption (user-ratified): active-segment reads anchored at the last compaction boundary are allowed; when a session has no compaction boundary the active segment starts at byte 0 and reading it fully is an allowed active-segment read, because compaction caps the active segment at model-context scale by construction. This exemption never applies to any session that has a compaction boundary, and never permits reading across a compaction boundary backwards. Exemption (user-ratified): rollback-candidate edge navigation may sequentially request bounded transcript segment pages across compaction boundaries until it finds the next candidate or reaches the history edge. Candidate-free pages remain transient and do not expand the existing bounded resident windows; each navigation attempt has a 20-second deadline that stops pagination and surfaces an error. This exemption applies only to an already-open rollback picker and does not authorize any other full-history reader, cumulative count, or resident transcript buffer.
- No duplicated code. Do not introduce duplicated utilities, functions, functionality, code paths, subsystems, implementations, helpers, classes, or any duplicated code. Reviewers must flag duplicated code as P0 finding. Agents must proactively scan their code before handing it off for any duplicated functionality to avoid penalty for violating this rule.
...
- Workflow Interrupt authority is exact live execution only. Offer and accept Interrupt if and only if an Exact Execution Scope is actively executing an agent loop or Script process. Durable Run/current-Node rows, Automatic Intents, runtime gates, Session records, and waiting-Question scopes never authorize Interrupt. If persistence and live execution disagree, fail at the Workflow Execution owner; do not add store-derived or client-derived Interrupt fallbacks.
...
- When you make server changes that make its contract or api incompatible with existing GUI/TUI clients', don't forget to raise the protocol version in ./shared/protocol/version.json. You may do so without explicit user approval as needed.
