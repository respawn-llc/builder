...
- No String or numeric IDs. All IDs in all code except internal dependencies must be UUIDs v7. Remove and refactor any string IDs and minimize usage of third-party string IDs. Do not introduce string IDs. Convert UUIDs to strings only where required by a third party API/dependency
