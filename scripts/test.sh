...
Environment:
  ...
  KENT_TEST_GO_PACKAGE_PARALLELISM
            ...
            capped at 8, and falls back to 4 when CPU detection is unavailable.
...
if [ "$default_go_test_package_parallelism" -gt 8 ]; then
    default_go_test_package_parallelism=8
fi
