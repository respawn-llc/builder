...
- [x] **Make admission-collision regressions deterministic.** Replace completion-window success assertions with a production-observable admission/lifecycle barrier that confirms competing activation and replacement have reached the contested Session boundary; timeouts remain failure guards only.
...
- [x] **Fix persisted raw user-turn JSON.** Trace the user-turn persistence/projection boundary that records the raw local JSON artifact, correct the source representation, and add an automated regression covering ordinary turn persistence and reopen without rendering-only sanitization.
...
- 2026-07-23: Review remediation removed the optional existing-session Store path, narrowed legacy plan override APIs to a create-new-only entry point, and updated all direct test compositions to provide the runtime Authority. The collision regressions now use the ResourceReady lifecycle barrier plus `TryBlockSessionStarts`, and the duplicate descriptor helpers are gone.
- 2026-07-23: Recovery diagnostics now project the typed artifact evidence through the gateway's normal error message. An OpenAI-compatible malformed text-only `error` stream event is projected as assistant output; a remote interactive regression reopens persisted records and verifies the ordinary user/assistant messages with no `LocalEntryRecord` artifact.
- 2026-07-23: Remediation verification passed: focused LLM, gateway, and remote interactive persistence regressions; full `cli/app`; five changed-path packages under `-race`; both deterministic collision regressions at `-count=100`; all Go packages with `KENT_TEST_GO_PACKAGE_PARALLELISM=6 ./scripts/test.sh server ./... -count=1`; `KENT_TEST_GO_PACKAGE_PARALLELISM=6 ./scripts/ci-check.sh all`; and `./scripts/build.sh server --output ./bin/kent`. The shorthand `./scripts/test.sh server -count=1` was not a valid all-package invocation in the current runner because its explicit Go argument replaces the default `./...`, so the package-explicit command above is the verified equivalent.
