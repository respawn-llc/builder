...
- [x] **30. Perform design review, browser QA, and repository-wide non-Rust verification.** Start an isolated server and `pnpm --dir apps/desktop dev:browser`; use the `qa-harness` skill if manual setup is required. Perform the `ui-design` screen review for the visual facts deliberately excluded from tests: exact accessible name, primary versus success treatment, circular progress, chip placement before Labels, canonical status-icon mapping, one-line ellipsis, Add/Remove appearance, and light/dark behavior. Also verify async Dependencies scroll, row/New Task replacement, optimistic Remove reconciliation, server-projected limit explanation, cross-Workflow event cases, Workflow deletion refresh, and Start/Move continuation. Then run full `./scripts/test.sh`, `pnpm --dir apps lint`, `pnpm --dir apps typecheck`, `pnpm --dir apps test`, and `./scripts/build.sh --output ./bin/kent`; do not build or test frozen Rust/TUI. **Done when:** design/browser observations are recorded, architecture lint and the full non-Rust suite pass, and the refreshed `./bin/kent` build succeeds.
