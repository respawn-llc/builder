...
- Keepers (ratified in tui-rebuild-plan.md): tui-core (green tests) and client-contracts. Condemned: tui-bin 5-layer loop onion (runtime_host/runtime_driver/runtime_work_loop/endpoint_*), tui-app 649-type message layer + 9.5K reducer.rs.
...
## Recon findings
- Workspace: crates/{client-contracts, test-support, tui-app, tui-bin, tui-core} + tools/manifest-check. Deps: tui-bin → tui-app → (tui-core, contracts); tui-core standalone.
- LOC: tui-app 53,416 (132 files), tui-bin 47,433 (117 files), tui-core 12,791 (34 files + 27 test files), contracts 4,041.
- tui-app and tui-bin have ZERO crate-level tests. Only tui-core/contracts are tested — the "keeper = green with tests" criterion condemns both big crates wholesale.
...
Delete crates/tui-app and crates/tui-bin ENTIRELY (~100,849 LOC). Keep tui-core, client-contracts, test-support, tools/manifest-check, build-support (integration harness used by keepers). No stub binary — Platform A creates tui-bin fresh. Quality bar (operator): "Rust tui will be impeccable. Code is dirt cheap, architecture and product quality are priceless."
...
- [x] Review findings addressed: P0 clippy (3 lints in tui-core fixed by refactor — EntryLayout extraction deduped the triplicated prefix/width computation, visual.rs 797→795 under its shrink-only baseline); P2 fixture: route_contract.rs + tui-route-contract.json DELETED (parity pin test of Go route table, banned test class); deny.toml comments stay out (no-comments rule, rationale in docs).
...
- [x] The obsolete Rust networking implementation and its copied payload corpus were removed; server networking is rebuilt under the dedicated network phase.
- [x] Green: ./scripts/test.sh tui exit 0; manifest-check 11/11 + repo policy OK; cargo-deny green with zero ignores; clippy --workspace --all-targets -D warnings clean; amend + force-push PR #501.
