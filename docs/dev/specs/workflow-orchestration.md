...
- The v2.5-to-v2.6.1 upgrade may perform a one-time provenance repair for legacy Current Nodes and active Fan-Out branches. The repair selects only one same-Task, same-Workflow, branch-compatible source association from the Node dictated by the frozen Context Source, ordered by the latest association timestamp not later than the target association when that timestamp exists. A missing candidate or timestamp tie remains unresolved legacy state.
