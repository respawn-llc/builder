...
- Workflow Task Start, Resume, and Move retry failed managed-worktree setup once before returning failure. Kent discards or recreates only an empty provisional root or one unchanged from its original checkout. Kent preserves operator and setup changes and otherwise reruns setup in place. Setup scripts must tolerate repeated execution.
...
- Manual Move has no Worktree Setup correlation or attempt-progress stream. CLI and Desktop show their ordinary pending state until the synchronous Move response returns. A successful Move includes any previous worktree retained while replacing the provisional target. One typed Move preparation-error envelope reuses the Worktree Setup failure contract and includes the final diagnostic, any primary retained worktree, and any previous retained worktree. Target-preparation failure may have no primary retained worktree; setup-script failure includes the retained primary worktree and setup script.
...
- The CLI task-start, task-resume, and task-move commands may select a concrete target for an unlocked task even when the workflow has a fixed policy. Task Approval and Task creation have no target override.
...
- CLI task start, resume, and move never prompt interactively. Selection-required output identifies the reason and concrete rerun flags. Task start exposes the same typed outcome in JSON.
- CLI Task Start against a Task that already left the Start Node directs the operator to Task Resume or Task Move.
