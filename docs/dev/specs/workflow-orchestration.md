...
- A Task with a legacy managed worktree and usable recorded `HEAD` continues to use that worktree when its root satisfies the server namespace and source Workspace separation rules. Kent identifies its observed commit as legacy provenance and does not present it as a known original branch point.
