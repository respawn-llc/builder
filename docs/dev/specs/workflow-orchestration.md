...
- Caller cancellation while Kent is delivering an automatic successor's target assignment stops only that caller's observation. Server-owned assignment delivery and successor handling continue unchanged; an independent delivery failure still follows the ordinary actionable interruption path.
...
- Kent never promotes a historical association through timestamps, Session identifiers, Assignee compatibility, or matching source proof during ordinary operation.
- The v2.5-to-v2.6.1 upgrade may perform a one-time provenance repair only when persisted state proves the exact Context Source independently of later Workflow edits. Other legacy state remains unresolved.
...
- Initial Task Start preparation failure is nonfatal and leaves the Task unapplied. Explicit Resume, user Interrupt, Resume or Move preparation failure, protocol-violation interruption, live-scope failure, outcome-less finalization, and post-commit interruption Task-event delivery diagnostics remain nonfatal.
...
- Initial Task Start materializes an Agent Current Node only in the same metadata transaction that records its exact Session binding, Session Task ownership, and exact Workflow Node and Transition Branch provenance.
