...
- Workflow Task Start, Resume, and Move retry failed managed-worktree setup once before returning failure. Kent treats a provisional worktree as unchanged for removal or recreation only when Git reports it clean and its current `HEAD` equals its immutable creation-base commit. A missing creation-base commit is not unchanged. Kent otherwise reruns setup in place. Setup scripts must tolerate repeated execution.
