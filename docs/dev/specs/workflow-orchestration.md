...
- Workflow Task Start, Resume, and Move retry failed managed-worktree setup once before returning failure. Kent recreates an empty or unchanged provisional worktree before retrying and otherwise reruns setup in place. Setup scripts must tolerate repeated execution.
...
- Canonical Resume offers Retry setup, Choose another Execution Target, and Cancel. Recovery after failed Task Start uses Task Resume. Cancel closes the dialog without resolving the interruption, so the canonical Resume control can reopen it.
