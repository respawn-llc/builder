...
- Successful Script completion applies exactly one durable Current Node transition during Script finalization before the Script Exact Execution Scope retires.
...
- Every Node Transition into an Agent Current Node materializes exactly one typed assignment for the target Session before its Exact Execution Scope begins. The first assignment accepted by an existing Active Session Runtime owns Agent-start exclusivity until its non-success result is delivered and its position clears or its Exact Execution Scope retires. An unrelated competing assignment fails before transcript or context mutation and follows the Workflow owner's ordinary start-failure handling. A successor may be accepted after exclusivity clears.
- Before the target Runtime accepts that Agent assignment, Workflow startup performs no Session clone, Session metadata change, Current Node Session binding, or provider allocation. Those effects belong only to the accepted winner. Winner preparation failure leaves no assignment transcript/context mutation and follows ordinary start-failure handling.
- Accepting the winning assignment opens human Send/Steer, settings, and ordinary Steering admission before winner preparation. Post-turn Queue submission remains unavailable because no Agent Turn has begun.
- Preparation or prepared-payload validation failure leaves the reserved position transition-occupying while Steering accepted behind that winner drains, then clears the position when the Runtime returns to Workflow control-only admission. A failure after Kent attempts the required durable assignment mutation follows the Runtime's common commit-certainty and fatal-loss contract instead.
- Applying the winning assignment makes the Workflow start pending. The Runtime's empty-drain decision releases start readiness, and only then may the Workflow owner admit and start exact execution. Without an Active Session Runtime, the Workflow owner persists the assignment through its dormant Session path, and Workflow Start/Resume alone constructs the Runtime, applies that assignment, drains accepted Steering, observes readiness, and starts exact execution.
- Assignment acknowledgement reports assignment application only and never authorizes a direct provider start. Steering accepted after the assignment and before the Runtime's empty-drain decision applies before the first provider request.
- The reserved Runtime start owns exclusivity until one short non-interruptible publication operation transfers it without a gap to the matching Agent Exact Execution Scope. Preparation creates no live Exact Execution Scope and does not make the Current Node running or interruptible. Publication revalidates the expected Current Node, target Runtime generation, reserved Workflow start, and absence of conflicting live execution before it durably admits the Current Node and makes the exact execution visible together. If another Workflow lifecycle operation changed the expected Current Node first, Kent discards the prepared start, records one typed unavailable or conflict result in the reserved position, preserves the winning Workflow state and successor behavior, and performs no interruption. The Runtime drains Steering already accepted behind that start, then one empty-drain decision clears the position and delivers the recorded result once. Publication does not separately clear or deliver it. If another publication precondition fails while the Current Node still matches, Kent discards the prepared start and attempts ordinary durable interruption; a definitely uncommitted interruption leaves it ready for retry. A committed admission makes the exact execution fully visible before publication ends. A definitely uncommitted admission makes no exact execution visible, leaves the Current Node ready for retry, and records failure in the reserved position while already accepted Steering drains. Indeterminate Workflow Store certainty is fatal. Manual Move and Task Interrupt that arrive during publication wait for running publication or failure, then use ordinary lifecycle ownership. A prepared Agent start does not authorize Task Interrupt.
- When Manual Move, Approval, completion, or another authoritative lifecycle operation creates an Agent successor targeting the same Runtime generation before the old start publishes, the initiating request makes one ordinary assignment attempt immediately after committing that successor.
- The request does not wait for old-result delivery or Runtime-position clear.
...
- An occupied old Runtime position returns Workflow-start-unavailable and ordinary start-failure handling interrupts that exact Current Node once.
...
- Kent adds no post-clear submission promise, clear wait, notification, listener, polling, caller-independent continuation, cleanup worker, second admission attempt, additional Runtime position, transferred exclusivity, ready-Current-Node resubmit operation, retry, or replay.
