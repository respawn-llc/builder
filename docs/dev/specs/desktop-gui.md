...
- OR is the default. The popup uses a compact `OR`/`AND` segmented control. `No labels` clears named selections and disables that control; selecting a named label clears `No labels` and restores the previously chosen mode. Clear removes the active filter and restores OR.
