...
- Prompt editing offers insertable chips for direct Parameters. Selecting one inserts `.Params.<parameter_key>` at the cursor, or at the end if the prompt is not focused.
- Informational chips explain previous-Transition references as `{{.Params.<transition_key>.<parameter>}}` and prior-Node output references as `{{.Nodes.<node_key>.<field>}}`. They do not insert text because the operator must choose the concrete Transition, Node, and field keys.
...
- A prompt can author a guaranteed-prior Node output reference as `.Nodes.<node_key>.<field>`. Workflow orchestration owns its availability and validation rules.
