...
- Failed rebinds do not create workspace bindings or change session ownership. Workflow-owned sessions cannot move across projects. Session provenance relationships are global and may cross project boundaries.
...
- Durable commit status owns runtime side effects and retry ownership. An uncommitted write error applies no projection or state and leaves the operation retryable. A durably committed write applies projection and state exactly once, consumes queued/handoff/control retry ownership, and is never appended again; any post-commit observer error is still returned to the caller.
...
- The limit applies across roles and to delegation initiated by workflow agents. Workflow scheduler-created node sessions begin at depth `0` and do not consume a delegation level.
