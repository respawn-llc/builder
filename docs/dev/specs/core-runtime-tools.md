...
- `server/runtimecommand` owns Runtime Command ordering and command lifecycle for one exact agent execution scope.
...
- Tool execution may retain typed start/end lifecycle machinery, but every transcript-visible or model-visible effect enters Runtime Command instead of a separate append path.
- Runtime Command covers all transcript-visible, non-queued messages with exactly one exception: markdown line-by-line streaming of agent commentary and final-answer responses. Any other exception must be user-approved and locked in this spec.
- Ordering, transcript visibility, ongoing/detail presentation, model visibility, dedupe, derived events, post-persist state updates, completion supersession, and retryable restoration are Runtime Command policy, not separate state machines.
- Transport idempotency is a thin adapter that maps request identity to one Runtime Command result. It does not own ordering, queueing, execution lifecycle, reconciliation, or persistence disposition.
...
- Successful Goal mutations emit typed runtime status updates carrying the projected Goal status state so frontends can update from Goal SSOT instead of inferring status from transcript feedback or execution lifecycle. Set, pause, Resume, complete, and clear emit updates; show/read-only operations do not.
...
- Goal mutations are Runtime Commands. Runtime Command owns overwrite validation, ordered in-memory application, status publication, persistence, and model awareness; no caller applies goal state through a parallel immediate path.
- Goal CLI never mutates session DB directly. It submits through the live Runtime Command RPC.
