...
- `config.toml` supports file-only `[skills]` boolean toggles for per-skill new-session enable/disable. Disabled individual skills remain visible in GUI/TUI and only affect model visibility.
- `[skills].enabled` is a reserved, case-insensitive switch for the entire skills subsystem and defaults to `true`. `[subagents.<role>.skills].enabled` overrides the policy for an independently resolved role; omission inherits the global policy, and an explicit `true` can re-enable skills for a role when the global policy is disabled. The switch does not create, hide, enable, or disable subagent roles in role catalogs or callability checks.
- When the effective skills policy is disabled, Kent does not scan or enumerate Kent skill roots, inject the model-visible skills reminder, or produce skill-discovery warnings. Operator inspection reports the subsystem as disabled without listing individual skills. Per-skill toggles remain configured and apply again when the subsystem is re-enabled.
- The effective skills policy is resolved from the current loaded configuration when an agent or subagent runtime is launched or resumed and when existing context reconstruction reevaluates model-visible notices. It is not persisted or snapshot-locked into the session contract.
- Every normalized case variant of `enabled` is reserved and cannot act as an individual skill toggle.
