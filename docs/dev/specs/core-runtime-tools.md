...
- Runtime activity is a server-owned live-state read model. Active/running state is resolved from the runtime-owned active step plus registry-owned runtime directory, close/drain, and start-admission state, not from session database records, run history events, transcript rows, mirrored registry active-run state, or client-local booleans.
- `clientui.RuntimeActivity` is the client-facing active/idle read model. It covers unavailable, starting/reserved, registered idle, running, awaiting prompt/approval, draining, and closing states, plus queue-accepting state, closed active kind for exclusive steps, and optional live run details.
- Active kind is assigned by the runtime execution owner at exclusive-step begin and carried on the active-step snapshot. Runtime activity projection copies that value; it does not infer user, workflow, goal, compaction, shell, background, or maintenance kind from client state, raw events, or caller-specific adapters. Every production exclusive-step call site must be mapped by a guard table.
- `shared/clientui` owns `RuntimeActivity` constructors and validation. Server packages compose runtime facts and call the shared constructors; they do not maintain a duplicate activity validation matrix.
- `clientui.ReadModelVersion` is the single per-session epoch/generation/sequence used by runtime activity, input reconciliation, main-view snapshots, interrupt responses, versioned runtime activity/reconciliation events carried on `SessionActivity`, and migrated prompt read-model stream events. Clients apply runtime UI payloads by this one version and ignore stale delayed responses or events.
- A registry-owned per-session read-model coordinator allocates `ReadModelVersion` values inside one snapshot/publication critical section and tracks the current sequence independently from retained stream events. Response-only sequence holes are valid stale-apply ordering points only; they are not replay cursors for the raw `SessionActivity` stream. The coordinator does not retain a second runtime activity event ring; runtime activity events are retained only by the existing raw `SessionActivity` broker.
- The read-model coordinator is session-scoped, not runtime-entry-scoped. Runtime entries and active subscribers pin their coordinator; dormant/no-runtime main-view hydration obtains a coordinator from a bounded LRU/TTL cache, and eviction/recreation uses a new generation so stale typed events are dropped. Dormant hydration returns unavailable activity plus requested input reconciliation under the same version.
- `clientui.RuntimeInputReconciliation` is keyed by `RuntimeOperationRef` and is delivered under the same `ReadModelVersion` as activity and read-model stream events. Input-reconciliation state can change while runtime activity stays running or idle; the shared version still orders the whole runtime UI read model atomically.
- Runtime activity/reconciliation events are carried on the existing `SessionActivity` stream with `ReadModelVersion` in the typed payload. The stream uses its raw sequence cursor for replay continuity; raw `AfterSequence` is never derived from `ReadModelVersion`, raw gaps hydrate from main view, and raw sequence never carries runtime liveness ordering.
- Initial attach and reconnect use independent cursors: hydrate main view to obtain the base `ReadModelVersion`, subscribe or resume `SessionActivity` with only the raw stream cursor, buffer typed activity/reconciliation events until hydration applies, then apply only same-generation events whose version sequence is newer than the hydrated/applied version. Epoch or generation mismatch triggers hydration/reset.
- Do not add a separate runtime UI read-model subscription for BUI-146. Existing session-stream attachment cleanup, partial-subscription-failure cleanup, gateway connection cleanup, subscriber accounting, and idle-unload policy remain the subscription/interest source for versioned runtime activity events.
- Runtime activity publications are coordinator-stamped for step and non-step transitions: prompt/approval begin and resolve, runtime registration, close/drain start, close/drain finish/unavailable, and start-marker clear/failure. Awaiting-prompt liveness is delivered by the runtime UI read model and is not deferred to prompt content stream migration.
- Terminal step publication is an admission barrier: after runtime clears active state for resolver reads and before `StepEnded` idle/unavailable publication commits, no successor or unrelated `RunWhenIdle` work may begin a new active step. New exclusive work waits, queues, or fails with the existing admission policy until the terminal read-model publication commits.
- Prompt initial snapshots are response-only coordinator snapshots stamped with `ReadModelVersion` after prompt stream migration; the snapshot version becomes the subscriber cursor. Prompt state resolved before the snapshot read appears in the snapshot, and prompt state resolved after the snapshot read is delivered as a higher-version retained prompt event.
- Main-view refresh requests carry the client's pending `RuntimeOperationRef` values, and main-view responses return an input-reconciliation snapshot keyed by operation ref. Clients use this typed snapshot for reconnect and stream-gap recovery instead of matching transcript text.
- Runtime interrupt responses return `RuntimeInterruptResponse{Version, Activity, InputReconciliation}` from one coordinator snapshot. Activity and input reconciliation in the response share the same `ReadModelVersion`; unknown or unavailable reconciliation is represented explicitly instead of omitting the field.
- Runtime UI read-model publication is an in-memory coordinator commit with no recoverable operational error path for valid owner snapshots. Invalid owner snapshots or DTO combinations are invariant violations handled by the shared invariant policy, not runtime liveness states or retry loops. Panic mode fails fast. Diagnostic mode records the violation and, for terminal step/close publications, advances `ReadModelVersion` with a safe idle/unavailable recovery activity or hydration-required signal so clients do not remain stuck on the previous running snapshot. Server diagnostics include session ID, publication cause, current/proposed read-model version when available, resolver inputs, owner snapshots, cached/resolved activity, and stack/trace.
- Input-bearing runtime client methods accept caller-created request IDs and `RuntimeOperationRef` values before dispatch. UI layers store the ref before starting command goroutines; shared clients and runtime clients validate and forward it instead of generating hidden input-operation IDs after dispatch.
- Session views, runtime-control responses, runtime activity events, worktree blocking, runtime release, runtime replacement, and idle-unload decisions consume the same runtime activity seam instead of recomposing liveness from separate run snapshots or database state.
- Runtime state is never tracked in the database. Persisted history must not contain liveness markers that clients or server projections use as active/idle truth.
- Crash/reopen model-context recovery may persist `PendingModelRecovery` from `model_recovery_pending` / `model_recovery_consumed` events so request assembly can add the interruption context required for prior incomplete tool calls. That marker is not runtime activity and must not drive active/idle decisions. Legacy `in_flight_step` metadata is a bounded-open-time recovery candidate only, not a liveness source or SQL-driven event migration.
- Runtime activity and goal status are separate server-produced facts. UI surfaces may combine them visually, but clients must not derive goal status from runtime activity or runtime activity from goal status.
- Prompt and approval details are owned by the prompt read model. Runtime activity may say an active run is awaiting prompt/approval so clients suppress ordinary spinners and keep liveness/control policy correct, but prompt contents and answers do not become runtime liveness sources.
- Forced local TUI exit detaches the local runtime owner reference. It does not release, force-close, or shut down a shared daemon runtime solely because that client exited. It persists visible draft input plus structured recovery buffers for hidden active/queued/injected/reviewer input before shutdown.
- Runtime owner-reference mutation has one server primitive used by release and forced local owner drop. Forced local exit uses the existing release route with an explicit close policy such as `detach_only`, which drops/orphans this owner without closing even when idle; normal exit uses the close-if-idle policy. A separate detach route is unnecessary unless implementation proves a semantic gap.
...
- Breaking runtime activity/control wire changes bump the shared protocol version or expose an explicit capability bit. Old daemon/client combinations fail with an actionable restart/incompatibility diagnostic instead of partially applying old DTOs.
- Runtime activity implementation must preserve package boundaries and decomposition guards: runtime remains execution owner, registry/read-model code does not import execution internals beyond typed seams, runtime-control service wiring does not own operation ledgers, and new behavior goes into focused files rather than existing large orchestration files.
...
- Projection and server read-model invariant checks use diagnostic mode by default and panic in global debug mode or when `KENT_INVARIANT_MODE=panic`; `KENT_INVARIANT_MODE=diagnostic` forces diagnostic recovery for local troubleshooting.
...
- `/goal resume` on an already active goal is a no-op and does not emit a duplicate goal-resumed transcript message.
...
- Ctrl+C during a runtime-owned `goal_loop` step keeps persisted status `active` and creates runtime-local suspension only. Interrupting a normal user turn, shell, compaction, workflow turn, or background step while a goal is merely active does not create suspension or auto-resume state; an already-suspended goal stays suspended. The next successful non-workflow user turn auto-resumes the suspended goal loop after that turn completes (no `/goal resume` needed); an explicit `/goal pause` is still the hard pause. A user turn that is itself interrupted leaves the loop suspended.
- Interrupt against an active but idle goal is a no-op for goal suspension. It does not create interrupted/resume-pending goal state when no live run or cancellable start exists.
- Goal interruption state is owned by the server runtime. Active plus interrupted means the goal remains active and resumes at the first available runtime opportunity; clients do not invent separate goal-suspension APIs or client-only goal resumption state. Pausing, completing, clearing, or replacing the goal cancels pending automatic resumption.
