...
- Client lifecycle hook commands are global client-local configuration and execute independently for every interactive controlling TUI attachment, including a TUI attached to a server on another machine. Desktop clients, unattended headless/subagent clients, and server-only processes do not execute lifecycle hooks, and a server never supplies or overrides the executable command.
- The server remains authoritative for lifecycle facts and runtime orchestration. The attached controlling TUI owns only supervision of its local read-only hook after accepting a server-owned lifecycle fact; server ownership, endpoint locality, execution paths, and `OwnsServer()` do not gate hook eligibility or transfer runtime lifecycle ownership to the client.
- Client hook configuration uses the `[hooks.client]` namespace and every client-hook payload declares `scope = "client"`. The `[hooks.server]` namespace and `scope = "server"` value are reserved for future server-hook work; this does not define or enable server hooks.
...
- Accepted human input before the completion fence supersedes pending workflow completion and continues the same exact execution generation. Input after the fence is rejected with a typed retryable result so the client restores its draft; it is never transferred to a successor run.
...
- Failed rebinds do not create workspace bindings or change session attachment. Sessions referenced by durable workflow-run provenance cannot move across projects. Session provenance relationships are global and may cross project boundaries.
...
- Interactive session creation is materialized by a successful `PlanSession` before TUI attachment preparation. The first user message or other agentic-loop trigger remains the first model-request boundary.
...
- Every successful automatic, manual, or handoff history replacement outside workflow execution regenerates active-goal continuation context in that same atomic replacement. The reminder is omitted for paused, completed, cleared, or absent goals and for every workflow run. Its initialization slot follows base/headless context and precedes workflow/worktree context; manual user carryover and handoff-specific future-agent context remain later.
...
- Outside workflow runs, active-goal continuation after history replacement is a Markdown-backed developer initialization message. It begins `Heads up: this session has an active goal. Goal text:`, inserts the exact goal verbatim between the ordinary nudge's `<goal>` markers, says `Continue working towards the goal from the resumed state.`, and then reuses the ordinary goal nudge's shared `Work mode` and `Completion discipline` guidance. Agent-facing continuation text never mentions compaction.
...
- Live run-control routes are thin transport adapters, not lifecycle authorities or `RunPrompt` continuations. Input routes submit Runtime Commands; stop and wait address the exact live execution authority. The adapters retain only transport request state and return typed results.
...
- Clients report the server-owned `RuntimeActivity` projection. Running, queued, and waiting states require exact generation-matched live evidence; clients do not synthesize active/busy fallback state from local requests or durable rows. Registered-idle runtimes accept idle operations.
