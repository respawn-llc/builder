...
- Interactive Go TUI lifecycle hooks are client-owned read-only side effects. Every local or remote controlling TUI executes its own global client-configured command; desktop, unattended headless, subagent, and server-only paths do not execute lifecycle hooks, and servers cannot supply or override the command.
- The existing `LiveRunResult` remains the server authority for a completed live-run group. The server publishes one live-only typed terminal projection containing status, result kind, no-final reason, final assistant message, runtime error, timestamps, and `work_performed`.
- `work_performed` becomes true when at least one completed step in the live-run group observes two or more existing tool-start events. All existing tool-start emissions count identically, including recovery emissions.
- Terminal projection is best-effort and independent of live-run completion. It does not add completion phases or tokens and does not delay waiter release, queue admission, queue completion, interruption, or successor scheduling.
- Lifecycle hooks add no RPC route, durable event, acknowledgement, retry, reconstruction, or server scheduling policy.
...
- `server/sessionruntime` owns the single exact live agent/script execution and resource authority, including exact scopes, generations, joined stop/finalization, and release. Script scopes remain sessionless.
- `server/runtimecommand` owns Runtime Command ordering and command lifecycle for one exact agent execution scope. It also owns the dormant goal-command use case explicitly authorized by the Runtime Output Boundary; `server/sessionruntime` supplies exact live/dormant resource admission, and `server/runtimecontrol` remains transport-only.
- `server/runtimecontrol` is a thin client transport adapter over those authorities and retains request/idempotency state only.
- `server/runtimeops` does not own a second ordering, persistence-disposition, reconciliation, or execution-lifecycle state machine.
...
- Goal management is CLI/runtime-owned. Kent must not add model-callable Goal tools, worktree tools, Task management tools, or Workflow tools outside workflow-controlled Sessions. New tool addition happens strictly after human approval and spec edits.

## Runtime Output Boundary

- Runtime owns one active conversation list/stateflow per session runtime.
- Runtime Command is the sole model-visible ordering authority within one exact agent execution scope. Human input, agent-originated workflow completion intent, goals, and technical input enter that authority as typed commands; prompt answers resolve their exact live waiter directly.
- `steer` submits one or more Runtime Commands to the next **step** boundary. `queue` submits the same typed commands after the **model turn** boundary. See terminology.md for differences between turns and steps.
- Tool execution may retain typed start/end lifecycle machinery, but every transcript-visible or model-visible effect for a session with an Active Session Runtime enters Runtime Command instead of a separate append path.
- Runtime Command covers all transcript-visible, non-queued messages for sessions with an Active Session Runtime, with exactly one exception: markdown line-by-line streaming of agent commentary and final-answer responses.
- Dormant goal mutation is the only approved persisted transcript exception outside Runtime Command. When `server/sessionruntime` atomically confirms that no current agent resource owns the session, `server/runtimecommand` may persist the durable goal transition and then append its single typed goal notice through the session event-log capability without creating a runtime. This exception is forbidden when an Active Session Runtime exists; all live goal effects remain Runtime Commands.
- For sessions with an Active Session Runtime, ordering, transcript visibility, ongoing/detail presentation, model visibility, dedupe, derived events, post-persist state updates, completion supersession, and retryable restoration are Runtime Command policy. The dormant goal exception is owned by the same `server/runtimecommand` authority and does not create a second state machine.
- Transport idempotency is a thin adapter that maps request identity to one `server/runtimecommand` result, whether the authority selected a live Runtime Command or the dormant goal exception. It does not own ordering, queueing, execution lifecycle, reconciliation, or persistence disposition.
- Pending user text accepted through steering coalesces at flush into one user message separated by blank lines; the coalesced message is a normal Runtime Command.
- Accepted human input before the Completion Fence supersedes pending Workflow completion and continues the same Exact Execution Scope. Input after the fence is rejected with a typed retryable result so the client restores its draft; it is never transferred to a successor current Node or Session execution.
...
- Bundled clients receive transcript, session-activity, and prompt-activity updates through one ordered transcript/runtime event feed. Separate `session.activity` and `prompt.activity` route families do not exist.
- History replacement is a typed output mutation owned by Runtime Command. Normal additions after replacement use the same command authority. Messages about handoffs emitted to TUI scrollback remain Runtime Commands; only the logical history-replacement algorithm itself is not a normal addition.
...
- Interactive runtime activation returns an exact session-resource attachment containing the session ID and positive resource generation. Release must echo that attachment; a stale release is idempotent and cannot detach or close a successor generation.
...
- Failed rebinds do not create workspace bindings or change Session attachment. Sessions associated with a Task's Workflow Nodes cannot move across projects. Session provenance relationships are global and may cross project boundaries.
- Session provenance uses two separate optional relationships. The previous-session relationship owns human navigation and derived-session provenance for `/new`, `/review`, rollback forks, and workflow clones. The parent-agent relationship owns only model-spawned subagent ancestry.
- Derived sessions preserve the source session's parent-agent ancestry while recording the immediate source as their previous session. Legacy generic parent links migrate only to previous-session provenance and do not imply subagent ancestry.
- Rebind uses the existing Session-maintenance boundary: it blocks new execution, waits for the current model step, pauses queued steering, and rejects a Session-owned background process. Cross-project rebind then renames the Session artifact and commits workspace/project ownership in one metadata transaction, renaming the artifact back if that transaction fails.
- When a session selected from the interactive picker has an attached `SessionExecutionTarget` whose available canonical workspace root differs from Kent's current workspace root, startup UI presents a prompt to rebind to the currently open destination. A detached historical workspace-root snapshot never triggers or supplies that rebind.
- Workspace relocation/rebinding is explicit user action; Kent does not infer auto-rebinds.
- Attached session location derives exclusively from SQLite project/workspace/worktree relations. A detached session may retain an explicit historical workspace-root snapshot, but that snapshot is not an attachment or execution-target fallback.
- Session launch exposes the selected session location through one server-owned typed `SessionExecutionTarget`; it does not expose a parallel raw workspace-root authority.
...
- Session Store is the sole append-certainty authority and exposes one `{committed, error}` result for commitment-sensitive mutations. Callers never infer commitment from event-log bytes, metadata state, observer success, or local projection.
- `committed=false` applies no projection or state and leaves the operation retryable. `committed=true` applies projection and state exactly once, consumes queued/handoff/control retry ownership, and is never appended again; a post-commit observer error is returned with `committed=true`.
- Session Store resolves partial-write and fsync outcomes into its append-certainty result. Callers do not own rollback or uncertainty handling, and Workflow completion and runtime-control adapters do not own persistence-disposition state machines.
...
- Root-level `max_subagent_depth` is TOML-only, defaults to `2`, and accepts integers from `0` through `30`. `0` disables model-originated child creation. Values outside the supported range fail configuration validation; values above `30` explain that Kent does not support recursion chains that deep.
- `max_subagent_depth` uses normal global-then-workspace configuration precedence. The active effective value is applied when a child launch is attempted and is not snapshotted into session metadata.
...
- Every successful automatic, manual, or handoff history replacement outside Workflow Execution regenerates active-Goal continuation context in that same atomic replacement. The reminder is omitted for paused, completed, cleared, or absent Goals and during workflow-controlled execution. Its initialization slot follows base/headless context and precedes workflow/worktree context; manual user carryover and handoff-specific future-agent context remain later.
- Active-goal continuation context is injected only when history replacement creates a new agent-visible resumed state. Opening or process-resuming an existing session without replacement does not inject it.
- A durably committed `history_replaced` event completes compaction even when a post-commit observer returns an error. Runtime applies the replacement, generation rotation, stale system/reviewer prompt-snapshot clearing, and retry-ownership effects exactly once, publishes `CompactionCompleted`, returns the observer error separately, and does not publish failed or retryable status, rebuild, or retry the committed compaction.
...
- Local compaction-summary generation uses automatic tool choice. Post-compaction Workflow generation derives the current executable Node's normal generation policy.
...
- Goal mutations are server-owned goal commands owned by `server/runtimecommand`. With an Active Session Runtime, Runtime Command owns overwrite validation, ordered in-memory application, status publication, persistence, and model awareness. Without an Active Session Runtime, the dormant goal-command use case persists the durable goal update and then persists the corresponding model-facing goal notice directly into the session transcript without starting a runtime.
...
- On the dormant path, the durable goal update is authoritative and is persisted before its model-facing notice. If the later notice persistence fails, the goal update remains committed and the error is surfaced; no cross-resource transaction, rollback, or automatic repair is required.
...
- Model-originated child launches are limited by parent-agent ancestry. A root session is depth `0`; with the default maximum `2`, `root -> subagent -> sub-subagent` is allowed and creation of the next child is rejected before the child becomes durable.
- The limit applies across roles and to delegation initiated by workflow agents. Workflow Execution-created node sessions begin at depth `0` and do not consume a delegation level.
- Opening or continuing an existing session is not blocked by the child-creation depth policy.
- Delegation ancestry traversal stops as soon as the configured limit proves that the requested child is disallowed and never walks more than `30` parent-agent links. Older `ErrSessionNotFound` ends the walk as a root; permission, I/O, corrupt-record, unavailable-store, and other operational or integrity failures reject the launch. A repeated session in the visited prefix is corrupted lineage: production rejects the launch and debug mode fails fast with diagnostics.
- A stale or missing immediate `KENT_SESSION_ID` remains an invalid caller context and fails with guidance to unset it or run from a live Kent shell.
- A blocked child launch returns a typed error carrying attempted depth and configured limit. Model-facing command output says: “You are already a subagent, so you shouldn't spawn more subagents to prevent overloading the machine and infinite recursion. Do not attempt to use subagents anymore and complete the task on your own”. JSON command mode exposes the stable `subagent_max_depth_exceeded` classification and numeric attempted-depth and limit fields.
- Subagent catalog/base-context visibility is unchanged by the depth policy, including when `max_subagent_depth = 0`. Launch-time rejection is the authoritative enforcement; catalog hiding requires consistent coverage of fresh, derived, cloned, resumed, and compaction-rebuilt contexts and is outside this change.
...
- Progress defaults to live mode (`--progress-mode=stderr`): every finalized user-visible assistant commentary or final message is emitted to stdout when committed, while lifecycle notices remain on stderr. New Sessions announce the real `{{.LaunchCommand}} run steer <session-id> "prompt"` command only after their execution becomes actively steerable; resumed Sessions do not announce it. Compaction-start and recoverable-failure notices remain visible, while routine tool/reviewer/completion status spam is omitted. `-q` and `--quiet` are aliases for `--progress-mode=quiet`, which preserves final-result-only output and suppresses lifecycle notices.
- `kent run steer <session-id> <message...>` submits a Runtime Command into another Session's Exact Execution Scope. It requires an explicit positional Session ID, rejects a target equal to the caller's `KENT_SESSION_ID`, and prints `ok` when accepted. The target headless Session prints `Steered message: <full text>` from the same accepted command and emits no later delivery notice. It fails when the target has no active scope and includes the equivalent `kent run --continue <session-id> <message>` hint; it never starts or queues an idle follow-up turn.
- `kent run stop <session-id>` interrupts any active shared runtime by Session ID, regardless of which client started it. It requires an explicit positional Session ID, rejects a target equal to the caller's `KENT_SESSION_ID`, prints `Stopped` when an interrupt is accepted, and prints `No active execution` as a successful no-op when the Session is already idle or nonexistent. Pending steering accepted by the stopped scope is not executed; if the runtime closes before it can be resumed, Kent emits a visible stopped/failed queued-message status before dropping the in-memory queue item.
- `kent run wait <session-id>` waits for the active shared runtime's final result. It requires an explicit positional Session ID, rejects a target equal to the caller's `KENT_SESSION_ID`, fails fast without printing final-answer content when no active execution is present, and mirrors ordinary `kent run` abnormal completion handling. Final-text output includes the ordinary continue hint; `--output-mode=json` uses the same result object shape as ordinary `kent run --output-mode=json`. Wait has no progress output.
- Live execution-control routes are thin transport adapters, not lifecycle authorities or `RunPrompt` continuations. Input routes submit Runtime Commands; stop and wait address the exact live execution authority. The adapters retain only transport request state and return typed results.
- Live execution-control commands accept only flags that are meaningful for the specific operation: `--persistence-root` for targeting the configured root, and `--output-mode=json` for `wait`. They do not accept workspace, model, provider, agent, timeout, tools, or progress flags.
- Headless stdin is not a steering channel. Parent agents steer live Sessions with explicit `kent run` control commands instead of relying on stdin line or chunk boundaries.
- Headless or workflow-controlled execution registers the Session's single shared runtime and drives it. Interactive activation for the same active Session resolves and attaches to that same shared engine as an equal full-control surface: live transcript/status, user steering and queued messages, prompt/approval answers, and every control operate against the shared runtime with no ownership, lease, or limited-control mode.
...
- A submission to a busy runtime enters Runtime Command and applies at the next permitted boundary. If the completion fence wins first, the client receives a typed retryable failure and restores the draft. Commands submitted while the exact scope is closing are rejected the same way.
- Clients report the server-owned `RuntimeActivity` projection. Running, queued, and waiting states require matching Exact Execution Scope evidence; clients do not synthesize active/busy fallback state from local requests or durable state. Registered-idle runtimes accept idle operations.
...
- Enter, leave, and deletion that switches the calling session submit typed Runtime Commands. The command returns an accepted/scheduled result without waiting for an active model step to finish; Runtime Command applies the transition in the next between-steps idle slot before queued user work and serializes it through the workspace lock. When no model step is active, the same command may apply immediately but retains scheduled/reminder semantics.
- Human-readable `worktree enter` and `worktree leave` confirmations explain that the transition is scheduled for the agent's next step and usually takes a few seconds; they omit the operation ID. `--json` returns the scheduled acknowledgement, including its operation ID, for automation.
- Runtime Command admits at most one pending worktree-transition command per exact scope. An identical transport retry maps to the original command result; a different transition command is rejected with typed pending state.
- Scheduled worktree transition failures do not retroactively change the command exit status. Attached clients observe typed completion or failure through the ordered transcript/runtime event feed, and Kent steers a typed failure notice into the affected session when runtime steering is available. Successful target changes remain authoritative only when the ordinary worktree reminder arrives.
- Pending worktree-transition commands are not persisted or resumed. Server shutdown or restart drops them; reconnecting clients refresh authoritative worktree status instead of hydrating a separate operation record or stream.
