...
- Unsupported terminals must ignore OSC 8 and retain the visible path without additional destination text. Added, deleted, unsuccessful, and nonexistent files must remain linked when their paths identify absolute local paths. Moved files must link to their destination paths.
