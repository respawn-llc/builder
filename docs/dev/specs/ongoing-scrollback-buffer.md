...
- Mutable-band physical geometry is owned by the optional last successfully written terminal size and retained height. An absent painted size or zero retained height means no mutable band exists and its previous physical band height is zero. Received resize targets and legacy width-rehydration debounce state are separate observations and never advance painted geometry before a successful repaint or reset.
...
- Rows vacated when temporary live-region content disappears remain mutable capacity until later immutable output consumes them. Prompt and picker appearance, updates, mode changes, and dismissal must not turn those rows into immutable blank output or trigger scratch rehydration.
- A height change preserves a positive retained band's previously painted top in physical screen coordinates while erasing and repainting. Terminal growth never creates mutable capacity when retained height is zero. A combined legacy width-and-height change that schedules scratch rehydration retains the old painted geometry until reset; reset erases the resize-adjusted old physical band at the target size, or zero rows when no retained band exists. A failed write commits neither painted size nor retained height.
...
- The complete allowed client-side state for this surface is this closed list. Additions require explicit user approval through a question before any code is written:
  ...
  - Mutable-band frame state: pending tool-call rows, spinner/animation state, input field, status line, retained mutable-band height, and optional last successfully painted terminal size. The observed resize target and existing debounce flags are pending terminal-input state, not painted geometry.
  - The current prompt and FIFO queue with existing prompt-local editor/cursor/freeform state; one active Markdown projection; at most one in-flight render request carrying current token, operation token, render identity, and immutable render source; and one latest desired immutable complete prompt candidate. Projection work is single-flight/latest-wins. No prompt phase, deferred delivery refresh, selection-normalization state, or queued-answer generation ownership is stored.
...
- Rehydration erases only the mutable band, using positive retained height and optional last successfully painted geometry adjusted to the target height; with no prior paint or zero retained height it erases and prompt-marks zero rows. It then reopens the subscription through the same session-open hydration path and appends the received active segment below existing scrollback. A successful reset commits the target terminal size and zero retained height. A failed reset preserves the prior painted geometry, starts no transcript reset, subscription/hydration continuation, or reset retry, and then follows the existing terminal-write error policy that exits the TUI. Rehydration never clears scrollback, reaches into emitted content, or compares the hydrated segment against anything. Duplicate-looking output after rehydration is acceptable and must not be suppressed.
