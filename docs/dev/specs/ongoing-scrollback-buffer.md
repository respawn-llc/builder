...
- Immutable writes use OSC 133 output semantics. Under the terminal-native repaint policy, the mutable band is one OSC 133 redrawable semantic-prompt region, so supporting terminals clear it before resize reflow and the resize event repaints it from mutable frame state. The legacy policy does not emit redrawable semantic-prompt metadata. Retired mutable rows return to output semantics before erase, so semantic-prompt marking never survives into immutable rows or permits history replay.
...
- Height changes leave the Immutable Area under native Scrollback ownership and repaint only the Mutable Band. A height increase must not create client-owned blank padding above committed history. When a height decrease needs room for the Mutable Band, committed rows must move into native Scrollback instead of being erased. Width changes use OSC 133 repaint only for exact Ghostty or kitty capability evidence outside tmux; all other environments use the legacy fallback.
