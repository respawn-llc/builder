...
Ordinary alt-screen transitions do not recreate the ongoing surface and do not rehydrate transcript history. Terminal resize recreates the ongoing surface for the new geometry. While resize events are settling, committed transcript state remains app-owned and native stable delivery waits. Once resize settles, native reprojects already-delivered ledger blocks for the new geometry and delivers pending appends from the authoritative transcript projection. Resize must not replay from physical-terminal emitted bytes, raw terminal caches, memoized render text, or alternate native-scrollback state.
...
Local append-only system notices that have already been emitted to native stable scrollback remain physical history even when a later authoritative transcript projection does not include them. If the authoritative projection shares the terminal-visible prefix before those local rows, native appends any authoritative suffix after the local rows; if the authoritative projection is only behind that local suffix, native writes nothing. Local system notices may be physically appended at the tail even when the app projection displays them before already-emitted rows, and later reconciliation treats those notices as already delivered without replaying them. Reviewer status, reviewer suggestion, and tool-result rows are committed transcript content, not local append-only rows. Other local rows inserted ahead of unmatched already-emitted rows are not appendable because native cannot insert them at that earlier position; production disables native output and debug fails fast. Native must not rewrite, delete, or replay local rows.
