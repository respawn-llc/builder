...
- Kent attempts to queue Allow commentary before the Approval answer. The prompt does not accept another action until that queue operation finishes.
- If Allow commentary creation succeeds, Kent delivers the Approval answer afterward. If creation fails, Kent restores the commentary as unsent composer text and delivers the Approval answer without waiting for commentary delivery or transient-notice expiry.
