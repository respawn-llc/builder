...
- If a prior interrupt is still pending but Runtime activity shows that another Agent Step has started, `Ctrl+C` interrupts that new Step instead of taking the forced local detach path.
