...
The version of the live Session resources used by an Exact Execution Scope. Replacing those resources advances the Resource Generation. No stale handle can mutate or close the replacement. Prompt notifications admitted before draining are best-effort delivery for the closing generation and may be dropped or arrive after the generation begins draining; reopening reloads authoritative prompt state.
