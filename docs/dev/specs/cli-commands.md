...
- When the configured workspace path is unavailable to the target server and does not resolve to an attached Workspace, Headless Run excludes unavailable remote Workspaces from selection.
- Headless Run automatically selects the remote Workspace when exactly one available candidate remains.
- Headless Run returns a selection-required failure when zero or more than one available candidate remains.
