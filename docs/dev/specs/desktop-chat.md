...
- Warnings, runtime errors, generic system notices, generic warnings, untyped notices, and reviewer errors start expanded. Runtime diagnostics start collapsed. Empty known developer context is omitted; empty unknown developer context is an expanded Diagnostic row that identifies the unknown source and integrity failure.
...
- If Kent cannot apply issued Reviewer feedback in the follow-up turn, Kent preserves the feedback row and fails the engine and Session. It adds no retry, rollback, or recovery guarantee for that failure.
...
- Processes is a single dense list with no detail view, inline output insertion, or log view. It preserves server ordering and shows state, process identifier, age, working-directory basename, one-line command, and latest nonempty output line. Running age is elapsed time from start to now; terminal age is elapsed time from start to finish.
- Processes renders the server-owned recent-output preview. Invalid shell-output bytes appear according to the core Process preview contract and do not hide the process row.
- Process termination needs no confirmation. It disables repeated activation and shows stopping while pending. Only killable processes show the accessible terminate action. While Processes is open and Desktop is in the foreground, it refreshes every 1.5 seconds. Automatic refresh is best-effort, stale rows remain visible, and Processes shows no notification if it fails. Loading, failure with Retry, and empty states use the standard compact list states.
...
- Lazy and materialized Chat use one exhaustive semantic settings mutation operation. Each request selects one Agent, Supervisor, Thinking, Fast, Questions, or Auto-compaction action. An accepted request reports whether durable state changed and returns complete available Settings and Context after the durable commit; it does not wait for an accepted live-Runtime update to apply. A typed current-state rejection returns the same complete available projections. Malformed requests, inaccessible or unknown Chats, authentication failures, persistence failures, failure to accept the live-Runtime update, and server faults remain ordinary operation errors.
...
- Desktop's global connection state owns server disconnection. Chat adds no banner, card, modal, or second reconnect indicator.
- While disconnected, the persistent warning notice remains visible, Chat keeps its last committed authoritative content, and every server mutation is unavailable. Composer and form text remain present.
