...
- Lazy and materialized Chat use one exhaustive semantic settings mutation operation. Each request selects one Agent, Supervisor, Thinking, Fast, Questions, or Auto-compaction action. An accepted request reports whether durable state changed and returns complete available Settings and Context; a typed current-state rejection returns the same complete available projections. Malformed requests, inaccessible or unknown Chats, authentication failures, persistence failures, and server faults remain ordinary operation errors.
