...
- User, committed assistant, and live assistant content use the same approved Streamdown 2.5.0 Markdown presentation. It uses Streamdown's built-in sanitized raw-HTML and link behavior. Completed supported code is syntax-highlighted and selectable; incomplete code remains selectable plain text until Streamdown reports the fence complete. Diagrams, math, and built-in Markdown controls are unavailable.
