...
- A dormant Session mutation persists without creating an Active Session Runtime. A live non-Agent mutation completes failure-prone preparation before persistence, durably commits the prepared value, then accepts one update for only the exact current Runtime. The request returns after acceptance. The Runtime applies accepted updates in acceptance order at the next between-Agent-Step boundary. Agent replacement follows the first-model-request lock.
...
- Session Name persists immediately while an Agent Step runs. Thinking, Fast Mode, Supervisor, Questions, and Auto-compaction can commit and return while an Agent Step runs, then apply to the live Runtime at the next between-step boundary. None of these changes enter Pending Work.
