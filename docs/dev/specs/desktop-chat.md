...
- When an attached Chat destination receives `provider_turn_state_invalid` or `provider_turn_state_conflict`, Desktop presents a warning through Chat's status-notification owner and creates no transcript row.
- Desktop gives each provider-state warning a localized explanation and recovery guidance to retry model work if behavior is unexpected. It never displays provider routing-state values.
- Provider-state warnings are live-only and best-effort. Desktop does not persist, replay, acknowledge, or promise delivery of them.
