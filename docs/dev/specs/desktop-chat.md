...
- Expanded flat rows use the transcript width without a nested island. They are borderless and neutral except for the semantic color of their leading status icon. Patch/Edit additionally colors addition/removal counts and diff lines with their corresponding semantic colors. Their compact line shows icon, optional localized type, compact summary, and available status or actions. The compact line toggles expansion without taking row-specific actions.
