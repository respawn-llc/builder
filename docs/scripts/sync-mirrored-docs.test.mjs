...
async function createMirroredRepoFixture() {
  ...
  await mkdir(path.join(docsRoot, "src", "content", "docs"), {
    recursive: true,
  });
  await mkdir(path.join(docsRoot, ".generated", "content", "docs"), {
    recursive: true,
  });
...
test("syncMirroredDocs writes generated mirrored docs and removes legacy copies", async () => {
  ...
  await writeFile(
    path.join(paths.legacyOutputDirectory, "docs.md"),
    "legacy docs\n",
    "utf8",
  );
  await writeFile(
    path.join(paths.deprecatedGeneratedOutputDirectory, "security.md"),
    "legacy security\n",
    "utf8",
  );
  ...
  assert.deepEqual((await readdir(paths.outputDirectory)).sort(), [
    "contributing.md",
    "docs.md",
    "security.md",
  ]);
  await assert.rejects(
    readFile(path.join(paths.legacyOutputDirectory, "docs.md"), "utf8"),
  );
  await assert.rejects(
    readFile(
      path.join(paths.deprecatedGeneratedOutputDirectory, "security.md"),
