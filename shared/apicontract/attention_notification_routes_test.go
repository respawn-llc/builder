...
func TestAttentionNotificationRouteContracts(t *testing.T) {
	...
	if session.RequestType != reflect.TypeOf(serverapi.AttentionSessionNotificationSubscribeRequest{}) {
		t.Fatalf("session attention notification route request = %v", session.RequestType)
	}
}
