...
func TestSessionLifecycleResultJSONRoundTrip(t *testing.T) {
	...
	tests := []struct {
		name string
		want SessionDirective
		json string
	}{
		{
			name: "stop",
			want: StopSessionDirective(),
			json: `{"kind":"stop"}`,
		},
		{
			name: "select session with current auth",
			want: SelectSessionDirective(SessionAuthPreparationKeepCurrent),
			json: `{"kind":"select_session","auth":"keep_current_auth"}`,
		},
		{
			name: "select session after reauthentication",
			want: SelectSessionDirective(SessionAuthPreparationReauthenticate),
...
func TestInitialInputPolicyPreservesEmptyOverride(t *testing.T) {
	override := OverrideStoredDraftSessionDraftDisposition("")
	if override.Kind() != SessionDraftDispositionOverrideStoredDraft {
	...
	restore := RestoreStoredDraftSessionDraftDisposition()
	if _, ok := restore.OverrideText(); ok {
		t.Fatal("restore-stored-draft policy exposed an override")
	}
}

func assertLifecycleResultVariant(t *testing.T, result SessionDirective) {
	t.Helper()
	switch result.Kind() {
