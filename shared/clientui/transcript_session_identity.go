...
func (i TranscriptSessionIdentity) Validate() error {
	...
	case ConversationFreshnessFresh, ConversationFreshnessEstablished:
	default:
...
func validatePresentSessionExecutionTarget(target SessionExecutionTarget) error {
	if SessionExecutionTargetIsZero(target) {
		return fmt.Errorf("present session execution target is empty")
	}
	target = NormalizeSessionExecutionTarget(target)
	switch target.WorkspaceAvailability {
	case ProjectAvailabilityAvailable,
		ProjectAvailabilityMissing,
		ProjectAvailabilityInaccessible:
		if target.WorkspaceID == "" {
			return fmt.Errorf("linked session execution target requires workspace id")
		}
	case ProjectAvailabilityUnlinked:
	default:
...
func validateSessionExecutionWorktree(worktree SessionExecutionWorktreeTarget) error {
	...
	switch ProjectAvailability(worktree.Availability) {
	case ProjectAvailabilityAvailable,
		ProjectAvailabilityMissing,
		ProjectAvailabilityInaccessible:
		return nil
