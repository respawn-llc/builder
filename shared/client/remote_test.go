...
func TestRemoteTranscriptHandoffReconnectsAndBecomesSessionRemote(t *testing.T) {
	var connectionCount atomic.Int32
	const capability = "test-session-reattach-capability"
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		defer func() { _ = ws.Close() }()
		switch connectionCount.Add(1) {
		case 1:
			acceptRemoteHandshake(t, ws)
			acceptRemoteProjectAttachment(t, ws, "workspace-source", "/source")
			var ignored []byte
			_ = websocket.Message.Receive(ws, &ignored)
		case 2:
			acceptRemoteHandshake(t, ws)
			acceptRemoteProjectAttachment(t, ws, "workspace-source", "/source")
			acceptRemoteSessionAttachmentOrClosed(t, ws, "project-source", "workspace-source", "/source")
		case 3:
			acceptRemoteHandshake(t, ws)
			acceptRemoteProjectAttachment(t, ws, "workspace-source", "/source")
			acceptRemoteSessionAttachmentOrClosed(t, ws, "project-source", "workspace-source", "/source")
			var req protocol.Request
			if err := websocket.JSON.Receive(ws, &req); err != nil {
				t.Errorf("receive transcript subscribe: %v", err)
				return
			}
			if req.Method != protocol.MethodSessionSubscribeTranscript {
				t.Errorf("subscribe method = %q, want transcript", req.Method)
				return
			}
			if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(req.ID, protocol.SubscribeResponse{})); err != nil {
				t.Errorf("send transcript subscribe response: %v", err)
				return
			}
			var ignored []byte
			_ = websocket.Message.Receive(ws, &ignored)
		case 4:
			acceptRemoteHandshake(t, ws)
			request := acceptRemoteSessionAttachmentOrClosed(t, ws, "project-target", "workspace-target", "/target")
			if request == nil || request.GetReattachCapability() != capability {
				t.Errorf("reattach request = %+v, want Session capability", request)
				return
			}
			var persist protocol.Request
			if err := websocket.JSON.Receive(ws, &persist); err != nil {
				t.Errorf("receive draft persistence: %v", err)
				return
			}
			if persist.Method != protocol.MethodSessionPersistInputDraft {
				t.Errorf("handoff method = %q, want draft persistence", persist.Method)
				return
			}
			if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(persist.ID, serverapi.SessionPersistInputDraftResponse{})); err != nil {
				t.Errorf("send draft persistence response: %v", err)
				return
			}
			var initial protocol.Request
			if err := websocket.JSON.Receive(ws, &initial); err != nil {
				t.Errorf("receive initial input: %v", err)
				return
			}
			if initial.Method != protocol.MethodSessionGetInitialInput {
				t.Errorf("promoted method = %q, want initial input", initial.Method)
				return
			}
			if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(initial.ID, serverapi.SessionInitialInputResponse{Input: "draft"})); err != nil {
				t.Errorf("send initial input response: %v", err)
			}
		default:
	...
	remote, err := DialRemoteURLForProject(context.Background(), "ws"+server.URL[len("http"):], "project-source")
	if err != nil {
		t.Fatalf("DialRemoteURLForProject: %v", err)
	}
	subscription, err := remote.SubscribeSessionTranscript(context.Background(), serverapi.TranscriptSubscribeRequest{SessionID: "session-1"})
	if err != nil {
		t.Fatalf("SubscribeSessionTranscript: %v", err)
	}
	...
	for {
		remote.mu.Lock()
		handoff := remote.draftHandoff
		dead := handoff != nil && handoff.remote.control.IsDone()
		remote.mu.Unlock()
		if dead {
			break
		}
		if time.Now().After(deadline) {
			t.Fatal("pre-move Session handoff connection did not close")
		}
	...
	if _, err := remote.PersistInputDraft(context.Background(), serverapi.SessionPersistInputDraftRequest{
		...
		t.Fatalf("PersistInputDraft through replacement handoff: %v", err)
	}
	destination, found, err := remote.TakeSessionHandoff(context.Background(), "session-1")
	if err != nil {
		t.Fatalf("TakeSessionHandoff: %v", err)
	}
	if !found {
		t.Fatal("Session handoff was not available for promotion")
	}
	...
	if !present || binding.ProjectID != "project-target" || binding.WorkspaceID != "workspace-target" {
		t.Fatalf("promoted Session binding = %+v present=%t", binding, present)
	}
	initial, err := destination.GetInitialInput(context.Background(), serverapi.SessionInitialInputRequest{SessionID: "session-1"})
	if err != nil {
		t.Fatalf("GetInitialInput on promoted Session remote: %v", err)
	}
	if initial.Input != "draft" {
		t.Fatalf("promoted Session draft = %q, want draft", initial.Input)
	}
}

func TestRemoteTranscriptSubscriptionFailureClosesUninstalledHandoff(t *testing.T) {
	var connectionCount atomic.Int32
	handoffClosed := make(chan struct{})
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		defer func() { _ = ws.Close() }()
		switch connectionCount.Add(1) {
		case 1:
		...
		case 2:
			acceptRemoteHandshake(t, ws)
			acceptRemoteProjectAttachment(t, ws, "workspace-1", "/workspace")
			acceptRemoteSessionAttachmentOrClosed(t, ws, "project-1", "workspace-1", "/workspace")
			var ignored []byte
			_ = websocket.Message.Receive(ws, &ignored)
			close(handoffClosed)
		case 3:
			...
			if err := websocket.JSON.Send(ws, protocol.NewErrorResponse(req.ID, protocol.ErrCodeInternalError, "subscribe failed")); err != nil {
				t.Errorf("send transcript failure: %v", err)
			}
		default:
			t.Errorf("unexpected connection")
		}
	...
	defer func() { _ = remote.Close() }()
	if _, err := remote.SubscribeSessionTranscript(context.Background(), serverapi.TranscriptSubscribeRequest{SessionID: "session-1"}); err == nil {
		t.Fatal("SubscribeSessionTranscript unexpectedly succeeded")
	}
	select {
	case <-handoffClosed:
	case <-time.After(2 * time.Second):
		t.Fatal("failed transcript subscription leaked its Session handoff")
	}
	remote.mu.Lock()
	handoff := remote.draftHandoff
	remote.mu.Unlock()
	if handoff != nil {
		t.Fatal("failed transcript subscription installed a Session handoff")
	}
}

func TestRemoteNonTranscriptSessionSubscriptionDoesNotPrepareDraftHandoff(t *testing.T) {
	var connectionCount atomic.Int32
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		defer func() { _ = ws.Close() }()
		switch connectionCount.Add(1) {
		...
		case 2:
			...
			var req protocol.Request
			if err := websocket.JSON.Receive(ws, &req); err != nil {
				t.Errorf("receive Question-history subscribe: %v", err)
				return
			}
			if req.Method != protocol.MethodSessionQuestionHistorySubscribe {
				t.Errorf("subscribe method = %q, want Question history", req.Method)
				return
			}
			if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(req.ID, protocol.SubscribeResponse{})); err != nil {
				t.Errorf("send Question-history subscribe response: %v", err)
				return
			}
			var ignored []byte
			_ = websocket.Message.Receive(ws, &ignored)
	...
	defer func() { _ = remote.Close() }()
	subscription, err := remote.SubscribeQuestionHistory(context.Background(), serverapi.QuestionHistorySubscribeRequest{
		SessionID:   "session-1",
		MaxHandoffs: 1,
	})
	if err != nil {
	...
	remote.mu.Unlock()
	if handoff != nil {
		t.Fatal("non-transcript Session subscription prepared a draft handoff")
	}
	if got := connectionCount.Load(); got != 2 {
		t.Fatalf("connection count = %d, want project control plus subscription", got)
	}
}
