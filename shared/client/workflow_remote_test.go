...
func TestRemoteWorkflowStartPreservesAlreadyStartedConflict(t *testing.T) {
	handlerErr := make(chan error, 1)
	...
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		...
		if err := expectWorkflowRemoteMethod(ws, &req, protocol.MethodWorkflowTaskStart); err != nil {
			handlerErr <- err
			return
		}
		if err := websocket.JSON.Send(ws, protocol.NewErrorResponseWithData(
			...
			handlerErr <- fmt.Errorf("send workflow Task Start conflict: %w", err)
			return
		}
		handlerErr <- nil
	}))
	...
	defer func() { _ = remote.Close() }()
	_, err = remote.StartWorkflowTask(context.Background(), serverapi.WorkflowTaskStartRequest{
		TaskID:           source.TaskID,
		SetupOperationID: serverapi.NewWorktreeSetupOperationID(),
	})
	var conflict *serverapi.WorkflowTaskStartConflictError
	if !errors.As(err, &conflict) || !reflect.DeepEqual(conflict, source) {
		t.Fatalf("StartWorkflowTask error = %T %+v, want %+v", err, err, source)
	}
	if err := <-handlerErr; err != nil {
		t.Fatal(err)
	}
...
func TestRemoteWorkflowMoveOmitsSetupCorrelationAndDecodesRetainedPreviousWorktree(t *testing.T) {
	handlerErr := make(chan error, 1)
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		defer func() { _ = ws.Close() }()
		var req protocol.Request
		...
		if err := websocket.JSON.Receive(ws, &req); err != nil {
			handlerErr <- fmt.Errorf("receive workflow move: %w", err)
			return
		}
		if req.Method != protocol.MethodWorkflowTaskMove {
			handlerErr <- fmt.Errorf("workflow move method = %q", req.Method)
			return
		}
		var fields map[string]json.RawMessage
		if err := json.Unmarshal(req.Params, &fields); err != nil {
			handlerErr <- fmt.Errorf("decode workflow move request: %w", err)
			return
		}
		if _, exists := fields["setup_operation_id"]; exists {
			handlerErr <- errors.New("workflow move request carried setup_operation_id")
			return
		}
		response := serverapi.WorkflowTaskMoveResponse{
			...
			Applied: &serverapi.WorkflowTaskMoveApplied{
				...
				RetainedPreviousWorktree: &serverapi.RetainedPreviousWorktree{
					Worktree: workflowRemoteRegisteredWorktree(),
				},
			},
		}
		if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(req.ID, response)); err != nil {
			handlerErr <- fmt.Errorf("send workflow move response: %w", err)
			return
		}
		handlerErr <- nil
	}))
	...
	defer func() { _ = remote.Close() }()
	response, err := remote.MoveWorkflowTask(context.Background(), serverapi.WorkflowTaskMoveRequest{
		TaskID:       "task-1",
		TargetNodeID: "node-2",
	})
	if err != nil {
	...
	if response.Applied == nil ||
		...
		response.Applied.RetainedPreviousWorktree.Worktree.Registered.Kent.CanonicalRoot != "/repo/feature" {
...
func workflowRemoteRegisteredWorktree() serverapi.WorktreeTopologyEntry {
