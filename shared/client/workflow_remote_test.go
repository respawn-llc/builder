...
func TestRemoteWorkflowTaskResumeRoundTripsExecutionTargetContinuation(t *testing.T) {
	handlerErr := make(chan error, 1)
	...
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		...
		if err := websocket.JSON.Receive(ws, &req); err != nil {
			handlerErr <- fmt.Errorf("receive workflow task resume: %w", err)
			return
		}
		if req.Method != protocol.MethodWorkflowTaskResume {
			handlerErr <- fmt.Errorf("workflow task resume method = %q", req.Method)
			return
		}
		var resumeRequest serverapi.WorkflowTaskResumeRequest
		if err := json.Unmarshal(req.Params, &resumeRequest); err != nil {
			handlerErr <- fmt.Errorf("decode workflow task resume: %w", err)
			return
		}
		if resumeRequest.SetupOperationID != setupOperationID ||
			resumeRequest.ExecutionTarget == nil ||
			resumeRequest.ExecutionTarget.Mode != serverapi.WorkflowExecutionTargetModeCustomRef ||
			resumeRequest.ExecutionTarget.CustomRef == nil ||
			*resumeRequest.ExecutionTarget.CustomRef != customRef {
			handlerErr <- fmt.Errorf("workflow task resume request = %+v", resumeRequest)
			return
		}
		response := serverapi.WorkflowTaskResumeResponse{
			...
			Applied: &serverapi.WorkflowTaskResumeApplied{
				CurrentNodes: []serverapi.WorkflowTaskCurrentNode{{NodeID: "node-agent"}},
			},
		}
		if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(req.ID, response)); err != nil {
			handlerErr <- fmt.Errorf("send workflow task resume response: %w", err)
			return
		}
		if err := websocket.JSON.Receive(ws, &req); err != nil {
			handlerErr <- fmt.Errorf("receive workflow task resume selection request: %w", err)
			return
		}
		if req.Method != protocol.MethodWorkflowTaskResume {
			handlerErr <- fmt.Errorf("workflow task resume selection method = %q", req.Method)
			return
		}
		if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(req.ID, serverapi.WorkflowTaskResumeResponse{
			...
			handlerErr <- fmt.Errorf("send workflow task resume selection response: %w", err)
			return
		}
		handlerErr <- nil
	}))
	...
	defer func() { _ = remote.Close() }()
	response, err := remote.ResumeWorkflowTask(context.Background(), serverapi.WorkflowTaskResumeRequest{
		TaskID:           "task-1",
	...
	if response.Outcome != serverapi.WorkflowExecutionTargetActionOutcomeApplied ||
		...
		response.Applied.CurrentNodes[0].NodeID != "node-agent" {
	...
	if selectionResponse.Outcome != serverapi.WorkflowExecutionTargetActionOutcomeSelectionRequired ||
		selectionResponse.SelectionRequired == nil ||
		selectionResponse.SelectionRequired.Reason != serverapi.WorkflowExecutionTargetSelectionReasonUnlockedPreparationFailed {
		t.Fatalf("ResumeWorkflowTask selection response = %+v", selectionResponse)
	}
	if err := <-handlerErr; err != nil {
		t.Fatal(err)
	}
