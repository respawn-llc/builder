...
func TestRemoteWorkflowTaskSessionsRoute(t *testing.T) {
	nextOffset := 25
	remote := newWorkflowResponseRemote(t, protocol.MethodWorkflowTaskSessionList, serverapi.WorkflowTaskSessionListResponse{
		TaskID: "task-1",
		WorkflowOffsetPage: serverapi.WorkflowOffsetPage[serverapi.WorkflowTaskSessionItem]{
			Items: []serverapi.WorkflowTaskSessionItem{{
				SessionID: "session-1",
				AgentRole: "coder",
				Status:    serverapi.WorkflowTaskSessionStatusQuestion,
			}},
	...
	if response.TaskID != "task-1" ||
		...
		response.NextOffset == nil || *response.NextOffset != nextOffset {
		t.Fatalf("response = %+v", response)
	}
}

func TestRemoteWorkflowTaskSessionsRejectsMismatchedTask(t *testing.T) {
	remote := newWorkflowResponseRemote(t, protocol.MethodWorkflowTaskSessionList, serverapi.WorkflowTaskSessionListResponse{
		TaskID:             "task-other",
	...
	if _, err := remote.ListWorkflowTaskSessions(context.Background(), serverapi.WorkflowTaskOffsetPageRequest{
		TaskID: "task-1",
	}); err == nil {
		t.Fatal("ListWorkflowTaskSessions accepted a mismatched Task ID")
	}
}
