...
func TestRemoteWorkflowProjectLabelReorderRoute(t *testing.T) {
	handlerErr := make(chan error, 1)
	labelIDs := []string{"11111111-1111-4111-8111-111111111111", "22222222-2222-4222-8222-222222222222"}
	server := httptest.NewServer(websocket.Handler(func(ws *websocket.Conn) {
		defer func() { _ = ws.Close() }()
		var req protocol.Request
		if err := websocket.JSON.Receive(ws, &req); err != nil {
		...
		if err := expectWorkflowRemoteMethod(ws, &req, protocol.MethodWorkflowProjectLabelReorder); err != nil {
			handlerErr <- err
			return
		}
		var reorderRequest serverapi.WorkflowProjectLabelReorderRequest
		if err := json.Unmarshal(req.Params, &reorderRequest); err != nil {
			handlerErr <- fmt.Errorf("decode reorder request: %w", err)
			return
		}
		if reorderRequest.ProjectID != "project-1" || !reflect.DeepEqual(reorderRequest.LabelIDs, labelIDs) {
			handlerErr <- fmt.Errorf("reorder request = %+v", reorderRequest)
			return
		}
		if err := websocket.JSON.Send(ws, protocol.NewSuccessResponse(req.ID, serverapi.WorkflowProjectLabelCatalogResponse{
			Catalog: serverapi.WorkflowProjectLabelCatalog{
				...
				Labels: []serverapi.WorkflowProjectLabel{
					{ID: labelIDs[0], Name: "First"},
					{ID: labelIDs[1], Name: "Second"},
				},
			},
		})); err != nil {
			handlerErr <- fmt.Errorf("send reorder response: %w", err)
			return
		}
		handlerErr <- nil
	}))
	...
	defer func() { _ = remote.Close() }()
	response, err := remote.ReorderWorkflowProjectLabels(context.Background(), serverapi.WorkflowProjectLabelReorderRequest{
		ProjectID: "project-1",
		LabelIDs:  labelIDs,
	})
	if err != nil {
	...
	if response.Catalog.ProjectID != "project-1" ||
		!reflect.DeepEqual(response.Catalog.Labels, []serverapi.WorkflowProjectLabel{
			{ID: labelIDs[0], Name: "First"},
			{ID: labelIDs[1], Name: "Second"},
		}) {
		t.Fatalf("reorder response = %+v", response)
	}
	if err := <-handlerErr; err != nil {
		t.Fatal(err)
	}
