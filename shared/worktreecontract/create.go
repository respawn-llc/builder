...
type CreateValidationKind string

const (
	CreateValidationBaseRefRequired     CreateValidationKind = "base_ref_required"
	CreateValidationBranchNameRequired  CreateValidationKind = "branch_name_required"
	CreateValidationBranchNameForbidden CreateValidationKind = "branch_name_forbidden"
)

type CreateValidationError struct {
	Kind       CreateValidationKind
	Diagnostic string
}
...
func ProjectCreateValidationOwner(err error, baseRefEditable bool) CreateErrorOwner {
	var validationErr *CreateValidationError
	if errors.As(err, &validationErr) &&
		validationErr.Kind == CreateValidationBaseRefRequired &&
		baseRefEditable {
		return CreateErrorOwnerBaseRef
	}
	return CreateErrorOwnerForm
}

func ValidateCreateSpec(baseRef string, createBranch bool, branchName string) error {
	baseRef = strings.TrimSpace(baseRef)
	branchName = strings.TrimSpace(branchName)
	if createBranch {
		if branchName == "" {
			return &CreateValidationError{
				Kind:       CreateValidationBranchNameRequired,
				Diagnostic: "branch name is required when create_branch=true",
			}
		}
		if baseRef == "" {
			return &CreateValidationError{
				Kind:       CreateValidationBaseRefRequired,
	...
	if branchName != "" {
		return &CreateValidationError{
			Kind:       CreateValidationBranchNameForbidden,
