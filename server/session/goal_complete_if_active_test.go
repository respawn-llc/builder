...
func TestCompleteGoalIfActiveRespectsGuard(t *testing.T) {
	...
	active, _, err := store.SetGoal("ship the rework", GoalActorUser)
	if err != nil {
	...
	goal, transitioned, _, err := store.CompleteGoalIfActive(active.ID, GoalActorSystem)
	...
	if _, _, _, err := store.SetGoalStatus(GoalStatusActive, GoalActorUser); err != nil {
		t.Fatalf("resume: %v", err)
	}
	if _, transitioned, _, err := store.CompleteGoalIfActive("some-other-id", GoalActorSystem); err != nil || transitioned {
	...
	completed, transitioned, _, err := store.CompleteGoalIfActive(active.ID, GoalActorSystem)
	if err != nil || !transitioned {
	...
	if _, transitioned, _, err := store.CompleteGoalIfActive(active.ID, GoalActorSystem); err != nil || transitioned {
