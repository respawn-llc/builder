...
func TestCompleteGoalIfActiveRespectsGuard(t *testing.T) {
	...
	if store.Metadata().Goal.Status != GoalStatusPaused {
		t.Fatalf("paused goal mutated to %q", store.Metadata().Goal.Status)
	}
	...
	if store.Metadata().Goal.Status != GoalStatusActive {
		t.Fatalf("active goal mutated to %q after id-mismatch", store.Metadata().Goal.Status)
	}
	...
	if completed.Status != GoalStatusComplete || store.Metadata().Goal.Status != GoalStatusComplete {
		t.Fatalf("goal not completed: %+v", store.Metadata().Goal)
	}
