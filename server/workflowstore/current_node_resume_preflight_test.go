...
func TestUpdateWorkflowEdgeBlocksUnlistedJoinProviderParameterEditForReadyDownstream(t *testing.T) {
	ctx, store, binding := newTestStoreContext(t)
	workflowID := createFanoutJoinWorkflow(t, ctx, store)
	linkWorkflow(t, ctx, store, binding.ProjectID, workflowID, true)
	task := createDefaultTask(t, ctx, store, binding.ProjectID)
	...
	var edge EdgeRecord
	for _, candidate := range request.Edges {
		if candidate.Key == "join_b" {
			edge = candidate
			break
		}
	}
	if edge.ID == "" {
		t.Fatal("join_b Edge not found")
	}
	edge.Parameters = append(edge.Parameters, workflow.Parameter{
		Key:         "risk",
		Description: "New derived requirement.",
	})

	if _, err := store.UpdateEdge(ctx, edge); !workflowGraphEditPolicyErrorHasBlocker(err, "active_transition_parameter_changed") {
		t.Fatalf("UpdateEdge error = %v, want active-transition-parameter blocker", err)
	}
}
