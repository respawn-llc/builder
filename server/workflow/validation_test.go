...
func TestJoinRetainedTargetAcceptsEqualFanoutSources(t *testing.T) {
	def := fanoutWorkflow(t)
	def.Nodes = append(def.Nodes,
	...
	joinTarget := edgeByIDForValidationTest(t, &def, "edge_join_done")
	joinTarget.TargetNodeID = "node_review"
	joinTarget.ContextMode = workflow.ContextModeContinueSession
	joinTarget.ContextSource = workflow.ContextSource{Kind: workflow.ContextSourcePreviousTargetOrNew}
	joinTarget.PromptTemplate = "Review."
	def.TransitionGroups = append(def.TransitionGroups,
...
func TestJoinRetainedTargetRejectsDivergentFanoutSources(t *testing.T) {
	def := fanoutWorkflow(t)
	def.Nodes = append(def.Nodes,
		testAgentNode(def.ID, "node_review", "review", "Review", workflow.NodeFields{SubagentRole: "coder"}),
	)
...
func TestJoinRetainedTargetKeepsFanoutInvocationsSourceIsolated(t *testing.T) {
	def := fanoutWorkflow(t)
	def.Nodes = append(def.Nodes,
		testAgentNode(def.ID, "node_review", "review", "Review", workflow.NodeFields{SubagentRole: "coder"}),
	)
	...
	def.TransitionGroups = append(def.TransitionGroups,
		workflow.TransitionGroup{
			WorkflowID: def.ID, ID: "group_review_split",
			SourceNodeID: "node_review", TransitionID: "repeat", DisplayName: "Repeat",
		},
	)
	def.Edges = append(def.Edges, workflow.Edge{
		WorkflowID: def.ID, ID: "edge_review_plan", Key: "plan",
		TransitionGroupID: "group_review_split", TargetNodeID: "node_plan",
		ContextMode:    workflow.ContextModeNewSession,
		PromptTemplate: "Plan again.",
	})
	def = normalizeWorkflowEdgeShape(def)
...
func TestSerialConvergenceDoesNotMergeContinuationSources(t *testing.T) {
	def := validWorkflow(t)
	def.Nodes = append(def.Nodes,
		testAgentNode(def.ID, "node_path_a", "path_a", "Path A", workflow.NodeFields{SubagentRole: "coder"}),
		testAgentNode(def.ID, "node_path_b", "path_b", "Path B", workflow.NodeFields{SubagentRole: "coder"}),
		testAgentNode(def.ID, "node_converged", "converged", "Converged", workflow.NodeFields{SubagentRole: "coder"}),
		testAgentNode(def.ID, "node_review", "review", "Review", workflow.NodeFields{SubagentRole: "coder"}),
	)
	def.TransitionGroups = append(def.TransitionGroups,
		workflow.TransitionGroup{WorkflowID: def.ID, ID: "group_agent_path_a", SourceNodeID: "node_agent", TransitionID: "path_a", DisplayName: "Path A"},
		workflow.TransitionGroup{WorkflowID: def.ID, ID: "group_agent_path_b", SourceNodeID: "node_agent", TransitionID: "path_b", DisplayName: "Path B"},
		workflow.TransitionGroup{WorkflowID: def.ID, ID: "group_path_a_converged", SourceNodeID: "node_path_a", TransitionID: "converge_a", DisplayName: "Converge"},
		workflow.TransitionGroup{WorkflowID: def.ID, ID: "group_path_b_converged", SourceNodeID: "node_path_b", TransitionID: "converge_b", DisplayName: "Converge"},
		workflow.TransitionGroup{WorkflowID: def.ID, ID: "group_converged_review", SourceNodeID: "node_converged", TransitionID: "review", DisplayName: "Review"},
		workflow.TransitionGroup{WorkflowID: def.ID, ID: "group_review_done", SourceNodeID: "node_review", TransitionID: "finish", DisplayName: "Done"},
	)
	def.Edges = append(def.Edges,
		workflow.Edge{WorkflowID: def.ID, ID: "edge_agent_path_a", Key: "path_a", TransitionGroupID: "group_agent_path_a", TargetNodeID: "node_path_a", ContextMode: workflow.ContextModeNewSession, PromptTemplate: "Path A."},
		workflow.Edge{WorkflowID: def.ID, ID: "edge_agent_path_b", Key: "path_b", TransitionGroupID: "group_agent_path_b", TargetNodeID: "node_path_b", ContextMode: workflow.ContextModeNewSession, PromptTemplate: "Path B."},
		workflow.Edge{WorkflowID: def.ID, ID: "edge_path_a_converged", Key: "converge_a", TransitionGroupID: "group_path_a_converged", TargetNodeID: "node_converged", ContextMode: workflow.ContextModeContinueSession, ContextSource: workflow.ContextSource{Kind: workflow.ContextSourceImmediateSource}, PromptTemplate: "Converge."},
		workflow.Edge{WorkflowID: def.ID, ID: "edge_path_b_converged", Key: "converge_b", TransitionGroupID: "group_path_b_converged", TargetNodeID: "node_converged", ContextMode: workflow.ContextModeContinueSession, ContextSource: workflow.ContextSource{Kind: workflow.ContextSourceImmediateSource}, PromptTemplate: "Converge."},
		workflow.Edge{WorkflowID: def.ID, ID: "edge_converged_review", Key: "review", TransitionGroupID: "group_converged_review", TargetNodeID: "node_review", ContextMode: workflow.ContextModeContinueSession, ContextSource: workflow.ContextSource{Kind: workflow.ContextSourcePreviousTargetOrNew}, PromptTemplate: "Review."},
		workflow.Edge{WorkflowID: def.ID, ID: "edge_review_done", Key: "done", TransitionGroupID: "group_review_done", TargetNodeID: "node_done", ContextMode: workflow.ContextModeNewSession},
	)
	def = normalizeWorkflowEdgeShape(def)
