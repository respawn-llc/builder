...
func TestMergeAttentionCandidatesPreservesQuestionStepIdentity(t *testing.T) {
	...
	firstID := liveQuestionAttentionID(sessionID, firstStepID, promptID)
	secondID := liveQuestionAttentionID(sessionID, secondStepID, promptID)
	if firstID == secondID {
	...
	items := mergeAttentionCandidates(
		attentionPageCursor{},
		[]attentionCandidate{{item: serverapi.WorkflowAttentionItem{
			ID:               firstID,
			OccurredAtUnixMs: 1,
		}}},
		[]attentionCandidate{{item: serverapi.WorkflowAttentionItem{
