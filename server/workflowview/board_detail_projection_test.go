...
func TestWorkflowPickerAndAttentionIncludeScriptPathDiagnostics(t *testing.T) {
	...
	if len(attention.Items) != 0 {
		t.Fatalf("attention items = %+v, want invalid workflow excluded from Inbox", attention.Items)
	}
}
...
func TestBoardAndTaskDetailProjectParallelBranchPlacements(t *testing.T) {
	...
	branchPage, err := view.board(t).ListNodeCards(ctx, serverapi.WorkflowBoardNodeCardsListRequest{
		ProjectID:   binding.ProjectID,
		WorkflowID:  string(workflowID),
		NodeID:      branchColumn.Node.NodeID,
		LabelFilter: serverapi.WorkflowTaskLabelFilterNone(),
	})
	if err != nil {
