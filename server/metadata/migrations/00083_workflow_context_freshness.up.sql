...
SET
    continuation_source_kind = CASE
        ...
        AND (
            task_current_nodes.entered_by_edge_id IS NULL
            OR (
                SELECT edge.context_mode
                FROM workflow_edges edge
                WHERE edge.id = task_current_nodes.entered_by_edge_id
            ) = 'new_session'
        )
        THEN 'deferred_self'
        WHEN (
    ...
    legacy_materialized = CASE
        ...
        AND task_current_nodes.session_id IS NULL
        AND task_current_nodes.entered_by_edge_id IS NOT NULL
        AND (
            SELECT edge.context_mode
            FROM workflow_edges edge
            WHERE edge.id = task_current_nodes.entered_by_edge_id
        ...
        json(
            CASE
                WHEN node.kind = 'agent'
                 ...
                 AND json_extract(
                     task_pending_approval_branches.effective_edge_configuration_json,
                     '$.context_mode'
                 ) = 'new_session'
