...
-- name: ClaimWorkspacePathKey :execrows
UPDATE workspaces
SET managed_worktree_path_key = sqlc.arg(managed_worktree_path_key)
WHERE id = sqlc.arg(id)
  AND managed_worktree_path_key IS NULL
  AND sqlc.arg(managed_worktree_path_key) IS NOT NULL
  AND length(trim(sqlc.arg(managed_worktree_path_key))) > 0;

-- name: ReleaseWorkspacePathKey :execrows
UPDATE workspaces
SET managed_worktree_path_key = NULL
WHERE id = sqlc.arg(id)
  AND managed_worktree_path_key = sqlc.arg(managed_worktree_path_key);

-- name: GetWorkspacePathKeyByID :one
