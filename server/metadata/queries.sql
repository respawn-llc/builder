...
-- name: InsertWorktreeOperation :execrows
INSERT INTO worktree_operations (
    ...
    sqlc.arg(expected_target_json),
    sqlc.arg(execution_mode),
    sqlc.arg(lifecycle_state),
    sqlc.arg(lifecycle_version),
    sqlc.narg(terminal_result_json),
    sqlc.narg(terminal_error_json),
    sqlc.arg(created_at_unix_ms),
    sqlc.arg(updated_at_unix_ms)
)
ON CONFLICT(operation_id) DO NOTHING;

-- name: GetWorktreeOperationByID :one
SELECT
    operation_id,
    payload_json,
    expected_target_json,
...
-- name: CompareAndSetWorktreeOperationLifecycle :execrows
UPDATE worktree_operations
SET
    lifecycle_state = sqlc.arg(next_lifecycle_state),
    lifecycle_version = lifecycle_version + 1,
    terminal_result_json = sqlc.narg(terminal_result_json),
    terminal_error_json = sqlc.narg(terminal_error_json),
    updated_at_unix_ms = sqlc.arg(updated_at_unix_ms)
WHERE operation_id = sqlc.arg(operation_id)
  AND lifecycle_state = sqlc.arg(expected_lifecycle_state)
  AND lifecycle_version = sqlc.arg(expected_lifecycle_version);

-- name: ListRecoverableWorktreeOperations :many
SELECT
    operation_id,
    payload_json,
    expected_target_json,
