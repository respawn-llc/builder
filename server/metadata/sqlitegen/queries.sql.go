...
const listProjectWorkflowAttentionCandidates = `-- name: ListProjectWorkflowAttentionCandidates :many
SELECT
    kind,
    id,
    project_id,
...
WHERE project_id = ?1
  AND (
      CAST(?2 AS INTEGER) = 0
      OR occurred_at_unix_ms < ?3
      OR (occurred_at_unix_ms = ?3 AND id < ?4)
  )
ORDER BY occurred_at_unix_ms DESC, id DESC
LIMIT ?5
`

type ListProjectWorkflowAttentionCandidatesParams struct {
	ProjectID              string
	CursorActive           int64
...
func (q *Queries) ListProjectWorkflowAttentionCandidates(ctx context.Context, arg ListProjectWorkflowAttentionCandidatesParams) ([]WorkflowAttentionCandidate, error) {
	rows, err := q.db.QueryContext(ctx, listProjectWorkflowAttentionCandidates,
		arg.ProjectID,
		arg.CursorActive,
