...
CREATE INDEX task_label_assignments_label_task_idx
	...
	requireQueryUsesIndex(t, db, listBoardColumnTaskCounts, "sqlite_autoindex_task_label_assignments_1", "[]", "none", "", "[]", nil, "[]", "project-1", "workflow-1")
	requireQueryUsesIndex(t, db, listBoardColumnTaskCounts, "task_label_assignments_label_task_idx", "[]", "none", "", "[]", nil, "[]", "project-1", "workflow-1")
}

func TestListBoardNodeTasksKeepsMembershipAndPaginationDatabaseBacked(t *testing.T) {
	db := openSQLiteFixture(t, ":memory:")
	t.Cleanup(func() { _ = db.Close() })
	if _, err := db.Exec(`
CREATE TABLE tasks (
...
CREATE TABLE workflow_task_status_records (
	...
	attention_types_json TEXT NOT NULL
		...
		t.Fatalf("create Board node query-plan fixture: %v", err)
	}
	args := []any{
		"[]",
		"project-1",
	...
	requireQueryUsesIndex(t, db, listBoardNodeTasks, "tasks_project_workflow_updated_idx", args...)
	requireQueryUsesIndex(t, db, listBoardNodeTasks, "sqlite_autoindex_task_label_assignments_1", args...)
	requireQueryUsesIndex(t, db, listBoardNodeTasks, "task_label_assignments_label_task_idx", args...)
}
