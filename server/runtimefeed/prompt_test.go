...
func TestPendingPromptRejectsInvalidQuestionOptions(t *testing.T) {
	...
	tests := []TranscriptPrompt{
		...
		func() TranscriptPrompt {
			prompt := base
			prompt.ApprovalOptions = []clientui.ApprovalDecision{clientui.ApprovalDecisionDeny}
			return prompt
		}(),
	}
...
func TestPendingPromptAcceptsTypedApprovalWithoutServerLabels(t *testing.T) {
	prompt := TranscriptPrompt{
		...
		CreatedAt: time.Unix(1_700_000_000, 0),
		ApprovalOptions: []clientui.ApprovalDecision{
...
func TestPendingPromptRejectsInvalidApprovalOptions(t *testing.T) {
	...
	tests := []TranscriptPrompt{
		...
		func() TranscriptPrompt {
			prompt := base
			prompt.ApprovalOptions = []clientui.ApprovalDecision{
				clientui.ApprovalDecisionDeny,
				clientui.ApprovalDecisionDeny,
			}
			return prompt
		...
		func() TranscriptPrompt {
			prompt := base
			prompt.ApprovalOptions = []clientui.ApprovalDecision{"unknown"}
			return prompt
		}(),
	}
	for _, prompt := range tests {
