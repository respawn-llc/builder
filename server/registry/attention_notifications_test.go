...
func TestRuntimeRegistryPublishesGenericPromptAttentionToSessionOnly(t *testing.T) {
	...
	if pending.Source != clientui.AttentionNotificationSourceLive || pending.Type != clientui.AttentionNotificationEventPending || pending.Pending.ID != "prompt:session-1:ask-1" || pending.Pending.Target.Kind != clientui.AttentionNotificationTargetSessionPrompt {
		t.Fatalf("pending event = %+v", pending)
	}
	if event, err := desktopSub.Next(shortRegistryContext(t)); err == nil {
		t.Fatalf("desktop received generic prompt event: %+v", event)
	}
	...
	if resolved.Type != clientui.AttentionNotificationEventResolved || resolved.ID != "prompt:session-1:ask-1" {
...
func TestRuntimeRegistryPublishesTaskQuestionBatchWithoutGenericResolve(t *testing.T) {
	...
	if pending.Pending.ID != "question_batch:run-1:batch-1" {
	...
	if resolved.Type != clientui.AttentionNotificationEventResolved || resolved.ID != "question_batch:run-1:batch-1" {
...
func TestRuntimeRegistrySkippedFirstTaskQuestionPreparesBatchBeforeMaterialization(t *testing.T) {
	...
	if err := registry.PrepareTaskQuestionBatch(*first.QuestionBatch, "session-1", first.AttentionTarget, first.AttentionPresentation, time.Now().UTC()); err != nil {
...
func TestRuntimeRegistrySessionAttentionSnapshotUsesPendingPromptStore(t *testing.T) {
	...
	if pending.Source != clientui.AttentionNotificationSourceSnapshot || pending.Pending.ID != "prompt:session-1:ask-1" {
...
func TestRuntimeRegistrySessionAttentionSnapshotPreservesTaskQuestionBatch(t *testing.T) {
	...
	if pending.Pending.ID != "question_batch:run-1:batch-1" || pending.Pending.Target.Kind != clientui.AttentionNotificationTargetTaskDetail {
