...
func TestWorktreeDeleteTransitionRejectsHumanInputAcceptedBeforeRuntimeAdmission(t *testing.T) {
	mainClient, mainStarted, releaseMain := newGatedHookClient(
		finalTextResponse("done"),
		llm.Response{},
	)
	...
	submitDone := make(chan error, 1)
	go func() {
		_, err := engine.SubmitUserMessage(t.Context(), "active turn")
		submitDone <- err
	}()
	select {
	case <-mainStarted:
	...
	humanDone := make(chan error, 1)
	go func() {
		_, accepted, err := engine.QueueUserMessageForActiveRun(t.Context(), "human input wins", nil)
		if err == nil && !accepted {
			err = errors.New("human input was not accepted")
		}
		humanDone <- err
	}()
	waitForAcceptedRuntimeOperationCount(t, engine, 1)
	...
	select {
	case err := <-transitionDone:
		if !errors.Is(err, ErrWorktreeDeleteBlockedByQueuedWork) {
			t.Fatalf("RunWorktreeDeleteTransition error = %v, want ErrWorktreeDeleteBlockedByQueuedWork", err)
		}
	case <-time.After(runtimeTestSynchronizationTimeout):
		t.Fatal("Worktree deletion did not reject accepted human input at Runtime admission")
	}
	if transitionRan {
		t.Fatal("Worktree deletion ran after human input was accepted first")
	}
	if err := <-submitDone; err != nil {
		t.Fatalf("SubmitUserMessage: %v", err)
	}
