...
func TestEmitCompactionStatusStillPublishesFailureEventWhenErrorPersistenceFails(t *testing.T) {
	...
	err := newCompactionPersistence(eng).emitStatus("step-1", EventCompactionFailed, compactionModeAuto, CompactionInitiatorAutomatic, "remote", "openai", nil, 2, "quota exceeded")
	if err == nil {
...
func TestRemoteCompactionRefreshesWorkflowTaskCommentCount(t *testing.T) {
	...
	if _, _, err := eng.compactNow(context.Background(), "step-1", compactionModeManual, CompactionInitiatorUserRequested, "", false); err != nil {
...
func TestRemoteCompactionTaskCommentCountErrorDoesNotReplaceHistory(t *testing.T) {
	...
	_, _, err := eng.compactNow(context.Background(), "step-1", compactionModeManual, CompactionInitiatorUserRequested, "", false)
...
func TestCompactionOmitsActiveGoalContinuationWhenGoalIsNotActive(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, _, err := engine.compactNow(context.Background(), "step-1", compactionModeManual, CompactionInitiatorUserRequested, "", false); err != nil {
				t.Fatalf("compactNow: %v", err)
			}
...
func TestCompactionReplacementPayloadEmbedsReinjectedBaseMetaAndManualCarryoverAtomically(t *testing.T) {
	...
	if _, _, err := eng.compactNow(context.Background(), "step-1", compactionModeManual, CompactionInitiatorUserRequested, "", true); err != nil {
		t.Fatalf("compactNow: %v", err)
	}
...
func TestCompactNowCompletesCommittedHistoryReplacementObserverFailure(t *testing.T) {
	...
	_, receipt, err := fixture.engine.compactNow(context.Background(), "step-compact", compactionModeManual, CompactionInitiatorUserRequested, "", false)
	if !receipt.Committed || !errors.Is(err, observerErr) {
