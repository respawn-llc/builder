...
func TestOperationalPendingWorkRejectsDuplicateIdentityWithoutMutation(t *testing.T) {
	engine := pendingWorkTestEngine(t, Config{Model: "gpt-5"})
	releaseMaintenance := pendingWorkTestHoldMaintenance(t, engine)
	operationID := serverapi.NewWorktreeOperationID()
	itemID := pendingWorkTestMust(t, func() (runtimeids.QueueItemID, error) {
	...
	executions := 0
	execute := func(context.Context) WorktreeApplicationResult {
		executions++
		return CommittedWorktreeApplication(nil)
	}
	if _, err := engine.ScheduleWorktreeTransition(
		...
		t.Fatalf("schedule first Worktree transition: %v", err)
	}
	...
	var conflict *serverapi.PendingWorkIdentityConflictError
	if !errors.Is(err, serverapi.ErrPendingWorkIdentityConflict) ||
		!errors.As(err, &conflict) ||
		conflict.ItemID != itemID {
		t.Fatalf("duplicate identity error = %T %v", err, err)
	}
	after := pendingWorkTestSnapshot(t, engine)
	if len(after.Items) != len(before.Items) ||
		len(after.Items) != 1 ||
		after.Items[0].ID != before.Items[0].ID ||
		after.Items[0].CanonicalInput != before.Items[0].CanonicalInput {
		t.Fatalf("Pending Work changed after duplicate admission: before=%+v after=%+v", before.Items, after.Items)
	}

	releaseMaintenance()
	waitEngineLifecycleTasks(t, engine)
	if executions != 1 {
		t.Fatalf("Worktree executions = %d, want 1", executions)
	}
}

func TestOperationalPendingWorkRejectsCrossKindIdentityCollision(t *testing.T) {
	engine := pendingWorkTestEngine(t, Config{Model: "gpt-5"})
	releaseMaintenance := pendingWorkTestHoldMaintenance(t, engine)
	operationID := serverapi.NewWorktreeOperationID()
	compactionID := pendingWorkTestMust(t, func() (runtimeids.CompactionRequestID, error) {
		return runtimeids.ParseCompactionRequestID(operationID.String())
	})
	itemID := pendingWorkTestMust(t, func() (runtimeids.QueueItemID, error) {
		return serverapi.PendingWorkItemIDFromWorktreeOperation(operationID)
	})
	...
	if !errors.Is(err, serverapi.ErrPendingWorkIdentityConflict) ||
		!errors.As(err, &conflict) ||
		conflict.ItemID != itemID {
		t.Fatalf("cross-kind identity error = %T %v", err, err)
	}
	after := pendingWorkTestSnapshot(t, engine)
	if len(after.Items) != len(before.Items) ||
		len(after.Items) != 1 ||
		after.Items[0].ID != before.Items[0].ID ||
		after.Items[0].Kind != runtimeinput.PendingWorkItemKindWorktreeTransition {
		t.Fatalf("Pending Work changed after cross-kind collision: before=%+v after=%+v", before.Items, after.Items)
	}
