...
func (e *Engine) localCompactionSummaryWithRepair(ctx context.Context, repairStepID *string, input []llm.ResponseItem, instructions string, mode compactionMode) (string, compactionOverflowRepairStats, error) {
	...
	summarize := func(w []llm.ResponseItem, canRepair bool) (string, []llm.ResponseItem, error) {
		summary, err := e.localCompactionSummaryFromWindow(ctx, repairStepID, locked, systemPrompt, w, instructions, requestTools, mode)
		...
		summary, err = e.localCompactionSummaryFromWindow(ctx, repairStepID, locked, systemPrompt, repairedWindow, instructions, requestTools, mode)
...
func (e *Engine) localCompactionSummaryFromWindow(ctx context.Context, stepID *string, locked session.LockedContract, systemPrompt string, window []llm.ResponseItem, instructions string, requestTools []llm.Tool, mode compactionMode) (string, error) {
	...
	for attempt := 0; ; attempt++ {
		...
		if stepID == nil {
			resp, err = e.generateRuntimeWithRetryClient(ctx, e.llm, req)
		} else {
