...
func TestCompleteIdleCurrentNodeKeepsMixedAgentScriptSuccessorOwnedWhenAgentAssignmentFails(t *testing.T) {
	cause := errors.New("Agent assignment append failed")
	source := currentNodeReferenceForControllerTest(t, "task-mixed-successor", "node-source")
	agent := currentNodeReferenceForControllerTest(t, "task-mixed-successor", "node-agent")
	script := currentNodeReferenceForControllerTest(t, "task-mixed-successor", "node-script")
	sourceNode := workflow.CurrentNode{
		Reference:  source,
		Scheduling: &workflow.CurrentNodeScheduling{State: workflow.CurrentNodeSchedulingReady},
	}
	...
	controller, err := NewCurrentNodeController(store, runner, authority, NewMutationPermit(), CurrentNodeControllerConfig{
		AgentConcurrency: 1,
		AssignmentSteerer: &recordingCurrentNodeAssignmentSteerer{
			outcomes: []currentNodeAssignmentSteerOutcome{{steerErr: cause}},
		},
	})
	if err != nil {
		t.Fatalf("new current node controller: %v", err)
	...
	}, "mixed Agent/Script successor lost Script ownership after Agent assignment failure")
	if _, interrupted := store.interruption(script); interrupted {
		t.Fatal("mixed Agent/Script successor made the healthy Script branch resumable")
	}
}

func TestAcceptedReadRaceCannotRemainStableAfterSuccessorOwnershipFailureReturns(t *testing.T) {
	cause := errors.New("assignment preparation failed")
	source := currentNodeReferenceForControllerTest(t, "task-read-race", "node-source")
	target := currentNodeReferenceForControllerTest(t, "task-read-race", "node-target")
	sourceNode := workflow.CurrentNode{
		Reference:  source,
		Scheduling: &workflow.CurrentNodeScheduling{State: workflow.CurrentNodeSchedulingReady},
	}
	...
	if _, interrupted := store.interruption(target); !interrupted {
		t.Fatal("accepted inert-active read race became the stable successor state after mutation returned")
	}
}
