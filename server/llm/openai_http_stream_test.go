...
func TestGenerateStream_EnrichesNormalizedProviderErrorsFromResponseHeaders(t *testing.T) {
	tests := []struct {
		name           string
		event          string
	...
	}{
		{
			...
			event:          `{"type":"response.failed","sequence_number":1,"response":{"id":"resp_1","created_at":1,"error":{"code":"context_length_exceeded","message":"too many tokens"}}}`,
			wantCode:       UnifiedErrorCodeContextLengthOverflow,
			wantRetriable:  true,
			wantProviderID: "openai-compatible",
		},
		{
			name:           "response incomplete",
			event:          `{"type":"response.incomplete","sequence_number":1,"response":{"id":"resp_1","created_at":1,"incomplete_details":{"reason":"max_output_tokens"},"output":[]}}`,
			wantCode:       UnifiedErrorCodeUnknown,
			wantRetriable:  false,
			wantProviderID: "openai-compatible",
		},
		{
			name:           "unrecognized structured error",
			event:          `{"type":"error","details":{"unexpected":"shape"},"sequence_number":1}`,
			wantCode:       UnifiedErrorCodeUnknown,
			wantRetriable:  true,
			wantProviderID: "openai-compatible",
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				w.Header().Set("x-openai-authorization-error", "authorization diagnostic")
				_, _ = fmt.Fprintf(w, "data: %s\n\ndata: [DONE]\n\n", test.event)
			}))
			t.Cleanup(server.Close)
			transport := newOpenAIStreamTestTransportForServer(server)

			_, err := transport.GenerateStreamWithEvents(context.Background(), OpenAIRequest{ToolChoiceMode: ToolChoiceModeAutomatic, Model: "gpt-5"}, StreamCallbacks{})
			if err == nil {
				t.Fatal("expected stream provider error")
			}
			var providerErr *ProviderAPIError
			if !errors.As(err, &providerErr) {
				t.Fatalf("error = %T %v, want ProviderAPIError", err, err)
			}
			if providerErr.ProviderID != test.wantProviderID || providerErr.Code != test.wantCode {
				t.Fatalf("provider error = %+v, want provider %q code %q", providerErr, test.wantProviderID, test.wantCode)
			}
