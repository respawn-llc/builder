...
func TestServiceListWorkflowTasksValidatesAndDelegates(t *testing.T) {
	...
	resp, err := service.ListWorkflowTasks(ctx, serverapi.WorkflowTaskListRequest{ProjectID: &projectID})
	if err != nil {
		t.Fatalf("ListWorkflowTasks: %v", err)
	}
	if resp.WorkflowID != workflowID || len(resp.Tasks) != 1 || resp.Tasks[0].TaskID != taskID {
