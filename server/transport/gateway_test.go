...
func TestGatewayUnaryDispatchDoesNotSerializeLaterRequests(t *testing.T) {
	entered := make(chan struct{})
	release := make(chan struct{})
	var releaseOnce sync.Once
	appCore, _ := newGatewayTestCore(t, true, true)
	defer func() { _ = appCore.Close() }()
	readiness := &blockingGatewayReadinessService{
		ServerStatusService: appCore.ServerStatusClient(),
		entered:             entered,
		release:             release,
	}
	gateway, err := NewGateway(
		&gatewayReadinessDependencies{Core: appCore, readiness: readiness},
	...
	for _, id := range []string{"blocked", "later"} {
		if err := websocket.JSON.Send(conn, protocol.Request{
			JSONRPC: protocol.JSONRPCVersion,
			ID:      id,
			Method:  protocol.MethodServerReadinessGet,
			Params:  mustJSON(t, serverapi.ServerReadinessRequest{}),
		}); err != nil {
		...
		if id == "blocked" {
			...
			case <-time.After(time.Second):
				t.Fatal("blocked unary handler was not entered")
			}
		}
	}

	if err := conn.SetDeadline(time.Now().Add(time.Second)); err != nil {
		t.Fatalf("set gateway response deadline: %v", err)
	}
	var response protocol.Response
	if err := websocket.JSON.Receive(conn, &response); err != nil {
		t.Fatalf("receive later unary response: %v", err)
	}
	if response.ID != "later" || response.Error != nil {
		t.Fatalf("first completed response = %+v, want successful later request", response)
	}

	releaseOnce.Do(func() { close(release) })
	if err := websocket.JSON.Receive(conn, &response); err != nil {
		t.Fatalf("receive released unary response: %v", err)
	}
	if response.ID != "blocked" || response.Error != nil {
		t.Fatalf("released response = %+v, want successful blocked request", response)
	}
}

func TestGatewayAuthMutationBarrierPrecedesLaterStatelessAuthRequiredRequest(t *testing.T) {
	entered := make(chan struct{})
	release := make(chan struct{})
	var releaseOnce sync.Once
	...
	defer func() { _ = appCore.Close() }()
	gateway, err := NewGateway(
		&gatewayAuthBootstrapDependencies{
			Core: appCore,
			bootstrap: &blockingGatewayAuthBootstrapService{
				AuthBootstrapService: appCore.AuthBootstrapClient(),
				entered:              entered,
				release:              release,
			},
		},
	...
	for _, request := range []protocol.Request{
		{
			JSONRPC: protocol.JSONRPCVersion,
			ID:      "complete-no-auth",
			Method:  protocol.MethodAuthCompleteBootstrap,
			Params:  mustJSON(t, serverapi.AuthCompleteBootstrapRequest{Mode: serverapi.AuthBootstrapModeNone}),
		},
		{
			JSONRPC: protocol.JSONRPCVersion,
			ID:      "create-workflow",
			Method:  protocol.MethodWorkflowCreate,
			Params:  mustJSON(t, serverapi.WorkflowCreateRequest{Name: "after no-auth"}),
		},
	} {
		if err := websocket.JSON.Send(conn, request); err != nil {
			t.Fatalf("send %s: %v", request.Method, err)
		}
		if request.ID == "complete-no-auth" {
			select {
			case <-entered:
			case <-time.After(time.Second):
				t.Fatal("auth.completeBootstrap did not enter the bootstrap service")
			}
		}
	}

	if err := conn.SetDeadline(time.Now().Add(150 * time.Millisecond)); err != nil {
		t.Fatalf("set blocked response deadline: %v", err)
	}
	var response protocol.Response
	if err := websocket.JSON.Receive(conn, &response); err == nil {
		t.Fatalf("received %+v before auth mutation completed", response)
	} else if networkErr, ok := err.(net.Error); !ok || !networkErr.Timeout() {
		t.Fatalf("receive while auth mutation blocked: %v", err)
	}
	if err := conn.SetDeadline(time.Time{}); err != nil {
		t.Fatalf("clear blocked response deadline: %v", err)
	}

	releaseOnce.Do(func() { close(release) })
	responses := make(map[string]protocol.Response, 2)
	for range 2 {
		if err := conn.SetDeadline(time.Now().Add(time.Second)); err != nil {
	...
	for _, requestID := range []string{"complete-no-auth", "create-workflow"} {
		if response := responses[requestID]; response.ID != requestID || response.Error != nil {
			t.Fatalf("%s response = %+v, want success", requestID, response)
		}
	}
}

type gatewayReadinessDependencies struct {
	*core.Core
	readiness apicontract.ServerStatusService
}

func (d *gatewayReadinessDependencies) ServerStatusClient() apicontract.ServerStatusService {
	return d.readiness
}

type blockingGatewayReadinessService struct {
	apicontract.ServerStatusService
	entered chan<- struct{}
	release <-chan struct{}
	once    sync.Once
}

func (s *blockingGatewayReadinessService) GetServerReadiness(ctx context.Context, req serverapi.ServerReadinessRequest) (serverapi.ServerReadinessResponse, error) {
	blocked := false
	s.once.Do(func() {
		blocked = true
		close(s.entered)
	...
	if blocked {
		...
		case <-s.release:
		case <-ctx.Done():
			return serverapi.ServerReadinessResponse{}, context.Cause(ctx)
		}
	}
	return s.ServerStatusService.GetServerReadiness(ctx, req)
}

type gatewayAuthBootstrapDependencies struct {
	*core.Core
	bootstrap apicontract.AuthBootstrapService
}

func (d *gatewayAuthBootstrapDependencies) AuthBootstrapClient() apicontract.AuthBootstrapService {
	return d.bootstrap
}

type blockingGatewayAuthBootstrapService struct {
...
func (s *blockingGatewayAuthBootstrapService) CompleteAuthBootstrap(ctx context.Context, req serverapi.AuthCompleteBootstrapRequest) (serverapi.AuthCompleteBootstrapResponse, error) {
	s.once.Do(func() {
		close(s.entered)
		select {
		case <-s.release:
	...
	if err := ctx.Err(); err != nil {
		return serverapi.AuthCompleteBootstrapResponse{}, context.Cause(ctx)
	}
	return s.AuthBootstrapService.CompleteAuthBootstrap(ctx, req)
}

func TestGatewaySubscriptionHandoffWaitsForPriorConnectionLaneAttachment(t *testing.T) {
	home := t.TempDir()
	...
	foreignSession, err := session.Create(
		filepath.Join(resolvedB.Config.PersistenceRoot, "projects", bindingB.ProjectID, "sessions"),
		filepath.Base(resolvedB.Config.WorkspaceRoot),
	...
	var releaseOnce sync.Once
	gateway, err := NewGateway(
		&blockingProjectExistsGatewayDependencies{Core: appCore, entered: entered, release: release},
		protocol.ServerIdentity{ProtocolVersion: protocol.Version, ServerID: "server-1"},
	)
	if err != nil {
		t.Fatalf("NewGateway: %v", err)
	...
	for _, request := range []protocol.Request{
		{
			...
			Params:  mustJSON(t, protocol.AttachProjectRequest{ProjectID: bindingB.ProjectID}),
		},
		{
			JSONRPC: protocol.JSONRPCVersion,
			ID:      "attach-session",
			Method:  protocol.MethodAttachSession,
			Params:  mustJSON(t, protocol.AttachSessionRequest{SessionID: foreignSession.Meta().SessionID}),
		},
		{
			JSONRPC: protocol.JSONRPCVersion,
			ID:      "subscribe-session",
			Method:  protocol.MethodSessionSubscribeTranscript,
			Params:  mustJSON(t, serverapi.TranscriptSubscribeRequest{SessionID: foreignSession.Meta().SessionID}),
		},
	} {
		if err := websocket.JSON.Send(conn, request); err != nil {
			t.Fatalf("send %s: %v", request.Method, err)
		}
		if request.ID == "attach-project" {
			select {
			case <-entered:
			case <-time.After(time.Second):
				t.Fatal("attach-project did not enter ProjectExists")
			}
		}
	}
	...
	for _, requestID := range []string{"attach-project", "attach-session"} {
		if response := responses[requestID]; response.ID != requestID || response.Error != nil {
			t.Fatalf("%s response = %+v, want success", requestID, response)
		}
	}
	subscriptionResponse := responses["subscribe-session"]
	if subscriptionResponse.ID != "subscribe-session" ||
		subscriptionResponse.Error == nil ||
		subscriptionResponse.Error.Code != protocol.ErrCodeStreamUnavailable {
		t.Fatalf("subscription response = %+v, want post-preflight stream-unavailable error", subscriptionResponse)
	}
}

type blockingProjectExistsGatewayDependencies struct {
	*core.Core
	entered chan<- struct{}
	release <-chan struct{}
	once    sync.Once
...
func (d *blockingProjectExistsGatewayDependencies) ProjectExists(ctx context.Context, projectID string) error {
	d.once.Do(func() {
		close(d.entered)
		select {
		case <-d.release:
	...
	return d.Core.ProjectExists(ctx, projectID)
}

var _ GatewayDependencies = (*gatewayReadinessDependencies)(nil)
var _ GatewayDependencies = (*gatewayAuthBootstrapDependencies)(nil)
var _ GatewayDependencies = (*blockingProjectExistsGatewayDependencies)(nil)
