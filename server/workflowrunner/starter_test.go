...
func TestWorkflowRuntimeTaskQuestionBatchPublishesOneTaskDetailAttention(t *testing.T) {
	...
	if resolved.Type != clientui.AttentionNotificationEventResolved || resolved.ID != pending.Pending.ID || resolved.Kind != clientui.AttentionNotificationKindQuestion {
...
func assertTaskQuestionBatchPending(t *testing.T, event clientui.AttentionNotificationEvent, fixture starterFixture, task workflowstore.TaskRecord, sessionID string, currentAskID string) {
	...
	if pending.Kind != clientui.AttentionNotificationKindQuestion || !strings.HasPrefix(pending.ID, "question_batch:") {
		t.Fatalf("pending id/kind = %q/%q", pending.ID, pending.Kind)
	}
	target := pending.Target
	if target.Kind != clientui.AttentionNotificationTargetTaskDetail ||
	...
	if pending.Question == nil || pending.Question.DisplayCount != 2 || pending.Presentation.Count != 2 {
		t.Fatalf("question state/presentation = %+v / %+v, want count 2", pending.Question, pending.Presentation)
	}
...
func (r *workflowAskHandlerRuntime) PrepareTaskQuestionBatch(batch askquestion.AskQuestionBatchMetadata, _ string, _ *clientui.AttentionNotificationTarget, _ *clientui.AttentionNotificationPresentation, _ time.Time) error {
