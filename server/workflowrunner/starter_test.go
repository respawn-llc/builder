...
func TestSchedulerRunsNoManagedWorktreeTargetFromSourceWorkspace(t *testing.T) {
	...
	target, err := fixture.metadata.ResolveSessionExecutionTarget(context.Background(), runs[0].SessionID)
	if err != nil {
		t.Fatalf("ResolveSessionExecutionTarget: %v", err)
	}
	if target.WorkspaceID != fixture.worktrees.workspaceID ||
		target.Worktree != nil ||
		target.EffectiveWorkdir != fixture.worktrees.metadataRoot() {
		t.Fatalf("session target = %+v, want source workspace %q at %q without worktree", target, fixture.worktrees.workspaceID, fixture.worktrees.metadataRoot())
	}
	if reminder := fixture.sessionMeta(t, runs[0].SessionID).WorktreeReminder; reminder != nil {
		t.Fatalf("worktree reminder = %+v, want none for source execution root", reminder)
	}
}
...
func TestWorkflowRuntimeQuestionBatchResumesSameSessionAndPublishesOneAttention(t *testing.T) {
...
func TestWorkflowAskHandlerApprovalUsesDurableTaskQuestionState(t *testing.T) {
	f := newWorkflowAskHandlerFixture()
	f.runtimes.response = askquestion.AskQuestionResponse{RequestID: "approval-1", Approval: &askquestion.AskQuestionApprovalPayload{Decision: askquestion.AskQuestionApprovalDecisionAllowOnce}}
	resp, err := f.handle(context.Background(), askquestion.AskQuestionRequest{
		ID:              "approval-1",
		Question:        "Approve?",
		Approval:        true,
	...
	if f.store.waitingAskID != "approval-1" || f.store.clearedAskID != "approval-1" {
		t.Fatalf("waiting ask lifecycle set=%q cleared=%q, want approval-1", f.store.waitingAskID, f.store.clearedAskID)
	}
	if len(f.runtimes.awaited) != 1 {
		t.Fatalf("awaited approval requests = %+v, want one", f.runtimes.awaited)
	}
	target := f.runtimes.awaited[0].AttentionTarget
	...
	if f.runtimes.cleared != nil || f.runtimes.skipped != nil {
		t.Fatalf("approval touched batch question attention cleared=%+v skipped=%+v", f.runtimes.cleared, f.runtimes.skipped)
	}
...
func TestWorkflowAskHandlerClearFailureLeavesTaskQuestionAttentionUnresolved(t *testing.T) {
	...
	f := newWorkflowAskHandlerFixture()
	f.runtimes.response = askquestion.AskQuestionResponse{RequestID: "ask-1", Answer: "done"}
	f.store.clearErr = clearErr
	_, err := f.handle(context.Background(), workflowTestBatchQuestion("ask-1", &batch))

	if !errors.Is(err, clearErr) {
		t.Fatalf("handleWorkflowAsk error = %v, want clear error", err)
	}
	if f.store.waitingAskID != "ask-1" || f.store.clearedAskID != "ask-1" {
		t.Fatalf("store calls waiting=%q cleared=%q", f.store.waitingAskID, f.store.clearedAskID)
	}
	if len(f.runtimes.cleared) != 0 {
		t.Fatalf("task question attention cleared despite durable clear failure: %+v", f.runtimes.cleared)
	}
}
...
func TestWorkflowAskHandlerSetWaitingAskFailureSkipsUnmaterializedBatchQuestions(t *testing.T) {
	...
	f := newWorkflowAskHandlerFixture()
	f.store.setErr = setErr
	_, err := f.handle(context.Background(), workflowTestBatchQuestion("ask-2", &batch))

	if !errors.Is(err, setErr) {
		t.Fatalf("handleWorkflowAsk error = %v, want set waiting ask error", err)
	}
	if len(f.runtimes.awaited) != 0 {
		t.Fatalf("runtime awaited prompt despite durable waiting ask failure: %+v", f.runtimes.awaited)
	}
	if len(f.runtimes.prepared) != 1 || f.runtimes.prepared[0] != "ask-2" {
		t.Fatalf("prepared batches = %+v, want current prompt preparation", f.runtimes.prepared)
	}
	if got, want := f.runtimes.skipped, []string{"ask-1", "ask-2", "ask-3"}; !reflect.DeepEqual(got, want) {
...
func TestWorkflowAskHandlerCancellationClearsAndSkipsRemainingBatch(t *testing.T) {
	...
	f := newWorkflowAskHandlerFixture()
	f.runtimes.err = context.Canceled
	_, err := f.handle(ctx, workflowTestBatchQuestion("ask-1", &batch))

	if !errors.Is(err, context.Canceled) {
		t.Fatalf("handleWorkflowAsk error = %v, want context.Canceled", err)
	}
	if f.store.clearedAskID != "ask-1" {
		t.Fatalf("cleared ask id = %q, want ask-1", f.store.clearedAskID)
	}
	if len(f.runtimes.cleared) != 1 || f.runtimes.cleared[0] != "ask-1" {
		t.Fatalf("cleared attention ids = %+v", f.runtimes.cleared)
	}
	if len(f.runtimes.skipped) != 1 || f.runtimes.skipped[0] != "ask-2" {
		t.Fatalf("skipped attention ids = %+v, want ask-2", f.runtimes.skipped)
	}
}
...
func TestWorkflowRuntimeInterruptReleasesBeforeInteractiveReactivation(t *testing.T) {
	workflowClient := newBlockingClient()
	interactiveClient := NewScriptedClient(
		llm.ProviderCapabilities{ProviderID: "fake", SupportsResponsesAPI: true},
		ScriptedFinalAnswer("ordinary final answer"),
	)
...
func TestWorkflowRuntimeCompactAndContinueReusesSourceSessionWithRealCompaction(t *testing.T) {
	fixture := newStarterFixture(t, config.WorkflowCompletionModeTool,
		ScriptedToolBatch("first comments", llm.ToolCall{
			ID:    "call-first-complete",
			Name:  "complete_node",
			Input: json.RawMessage(`{"commentary":"first comments","prior_summary":"first summary"}`),
		}),
		ScriptedFinalAnswer("compacted prior work summary"),
		ScriptedToolBatch("second done", llm.ToolCall{
	...
	if reqs[0].ToolChoiceMode != llm.ToolChoiceModeRequired ||
		reqs[1].ToolChoiceMode != llm.ToolChoiceModeAutomatic ||
		reqs[2].ToolChoiceMode != llm.ToolChoiceModeRequired {
		t.Fatalf("tool choice modes = %q, %q, %q, want required, automatic, required", reqs[0].ToolChoiceMode, reqs[1].ToolChoiceMode, reqs[2].ToolChoiceMode)
	}
...
func TestWorkflowRuntimeFanoutCompactAndContinueClonesUseBranchTransitionMetadata(t *testing.T) {
	...
	for _, node := range def.Nodes {
		nodeKeyByID[workflow.NodeIDOf(node)] = workflow.NodeKey(node)
	}
	task := fixture.createStartedTask(t)
...
func TestWorkflowRuntimeResumeInterruptedRunUsesSameSession(t *testing.T) {
	fixture := newStarterFixture(t, config.WorkflowCompletionModeTool, ScriptedToolBatch("resumed", llm.ToolCall{
		ID:    "call-complete",
		Name:  "complete_node",
		Input: json.RawMessage(`{"commentary":"resumed"}`),
	}))
	task := fixture.createStartedTask(t)
...
func TestStartWorkflowRunWaitsForLockedTaskWorktreeRestoreBeforeRefreshingRunContext(t *testing.T) {
	...
	starter := &Starter{store: &recordingRuntimeStore{runStartContext: workflowstore.RunStartContext{
		ExecutionRoot: &workflowstore.ExecutionRoot{
			SourceWorkspaceID:   "workspace-1",
			SourceWorkspaceRoot: "/workspace",
			Managed:             &workflowstore.ManagedExecutionRoot{WorktreeID: "worktree-1", Root: "/worktree"},
		},
	}}, worktrees: ensurer}
...
type blockingStarterTaskWorktreeEnsurer struct {
	started chan<- LockedTaskWorktreeRestoreRequest
	release <-chan struct{}
	err     error
}

func (e blockingStarterTaskWorktreeEnsurer) RestoreLockedTaskWorktree(_ context.Context, req LockedTaskWorktreeRestoreRequest) error {
	e.started <- req
	<-e.release
	return e.err
}
...
func (f starterFixture) createStartedTask(t *testing.T) workflowstore.TaskRecord {
	...
	if err := f.worktrees.RestoreLockedTaskWorktree(context.Background(), LockedTaskWorktreeRestoreRequest{TaskID: task.ID}); err != nil {
...
func (f starterFixture) sourceExecutionRoot() *workflowstore.ExecutionRoot {
	return &workflowstore.ExecutionRoot{
		SourceWorkspaceID:   f.worktrees.workspaceID,
		SourceWorkspaceRoot: f.worktrees.metadataRoot(),
	}
}
...
func newWorkflowQuestionBatchStarterFixture(t *testing.T) starterFixture {
	t.Helper()
	fixture := newStarterFixture(t, config.WorkflowCompletionModeTool,
		...
		ScriptedToolBatch("complete", llm.ToolCall{ID: "call-complete", Name: "complete_node", Input: json.RawMessage(`{"commentary":"answered both and finished"}`)}),
	)
...
type workflowAskHandlerFixture struct {
	starter  *Starter
...
func workflowTestBatchQuestion(askID string, batch *askquestion.AskQuestionBatchMetadata) askquestion.AskQuestionRequest {
	return askquestion.AskQuestionRequest{ID: askID, Question: "Proceed?", Origin: askquestion.AskQuestionOriginModelTool, QuestionBatch: batch}
}
...
func (e *metadataTaskWorktrees) RestoreLockedTaskWorktree(ctx context.Context, req LockedTaskWorktreeRestoreRequest) error {
	...
	_, statErr := os.Stat(worktreeRoot)
...
func (e *metadataTaskWorktrees) executionTargetCandidate(task workflowstore.TaskRecord) workflowstore.ExecutionTargetCandidate {
	requestedRef := "HEAD"
	commitOID := "fixture-commit-" + string(task.ID)
	return workflowstore.ExecutionTargetCandidate{
		...
		Root: workflowstore.ExecutionRoot{
			SourceWorkspaceID:   e.workspaceID,
			SourceWorkspaceRoot: e.metadataRoot(),
			Managed: &workflowstore.ManagedExecutionRoot{
				WorktreeID: "worktree-" + string(task.ID),
				Root:       filepath.Join(e.root, task.ShortID),
			},
		},
	}
}

func (e *metadataTaskWorktrees) metadataRoot() string {
	workspace, err := e.metadata.GetWorkspaceByID(context.Background(), e.workspaceID)
	if err != nil {
		e.t.Fatalf("GetWorkspaceByID: %v", err)
	}
...
func (c *blockingClient) Requests() []llm.Request {
	c.mu.Lock()
	defer c.mu.Unlock()
	return append([]llm.Request(nil), c.reqs...)
}
