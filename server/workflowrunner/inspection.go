...
func BuildPersistedWorkflowInspection(ctx context.Context, app config.App, sessionStore *session.Store, store *workflowstore.Store) (PersistedWorkflowInspection, error) {
	...
	if input.ExecutionRoot == nil {
		return PersistedWorkflowInspection{}, errors.New("workflow run execution root is required")
	}
	if err := input.ExecutionRoot.Validate(); err != nil {
		return PersistedWorkflowInspection{}, fmt.Errorf("validate workflow execution root: %w", err)
	}
	app.WorkspaceRoot = strings.TrimSpace(input.ExecutionRoot.SourceWorkspace.Root)
	...
	return PersistedWorkflowInspection{
		...
		WorktreeRoot: input.ExecutionRoot.EffectiveRoot,
	}, nil
}
