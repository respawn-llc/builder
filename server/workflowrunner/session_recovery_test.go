...
func TestSchedulerRejectedReviewContinueSessionAdmitsBeforeStartingLaterScriptIntent(t *testing.T) {
	...
	if len(runs) != 3 ||
		...
		runs[2].StartedAt != nil ||
		runs[2].SessionID != "" {
		t.Fatalf("blocked continuation run = %+v, want unstarted until Session/runtime admission completes", runs)
	}
	...
	case <-time.After(5 * time.Second):
		t.Fatal("scheduler Process did not return after continuation preparation release")
	}
	runs, err = fixture.store.ListRuns(ctx, task.ID)
	if err != nil {
	...
	if len(runs) != 3 ||
		...
		runs[2].EffectiveCompletionMode == nil ||
		runs[2].InterruptedAt != nil {
		t.Fatalf("continuation run after scheduler Process = %+v, want atomic Session/runtime admission", runs)
	}
	scriptRuns, err = fixture.store.ListRuns(ctx, scriptTask.ID)
	if err != nil {
