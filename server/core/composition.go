...
func NewWithContextOptions(ctx context.Context, cfg config.App, authSupport serverbootstrap.AuthSupport, runtimeSupport serverbootstrap.RuntimeSupport, opts Options) (*Core, error) {
	...
	workflowTaskStatusSnapshots, err := workflowview.NewTaskStatusSnapshotCoordinator(
		metadataStore,
	...
	workflowTaskSearch, err := workflowview.NewTaskSearch(workflowTaskProjector, workflowTaskStatusSnapshots)
	if err != nil {
		cleanupNewFailure()
