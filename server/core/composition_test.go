...
func TestComposedWorkflowTaskResumesAfterTerminalInterruptAndFinalAnswer(t *testing.T) {
	releaseInitialResponse := make(chan struct{})
	blockResumePreparation := make(chan struct{})
	resumePreparationEntered := make(chan struct{})
	releaseResumePreparation := make(chan struct{})
	releaseResumedResponse := make(chan struct{})
	var resumePreparationOnce sync.Once
	var releaseResumePreparationOnce sync.Once
	var releaseResumedResponseOnce sync.Once
	t.Cleanup(func() {
		releaseResumePreparationOnce.Do(func() { close(releaseResumePreparation) })
		releaseResumedResponseOnce.Do(func() { close(releaseResumedResponse) })
	})
	client := scriptedllm.NewClient(scriptedllm.Script{
		Capabilities: &llm.ProviderCapabilities{ProviderID: "scripted-terminal-resume", SupportsResponsesAPI: true},
		Steps: []scriptedllm.Step{
			{
				BeforeResponse: func(ctx context.Context) error {
					select {
				...
				Response: scriptedllm.FinalAnswer(`{"commentary":"interrupted workflow response"}`).Response,
			},
			scriptedllm.FinalAnswer(`{"commentary":"terminal final answer"}`),
			{
				BeforeResponse: func(ctx context.Context) error {
					select {
					case <-releaseResumedResponse:
						return nil
					case <-ctx.Done():
						return ctx.Err()
					}
				},
				Response: scriptedllm.FinalAnswer(`{"commentary":"resumed workflow response"}`).Response,
			},
		},
	})
	fixture := newComposedWorkflowFixture(t, composedWorkflowFixtureOptions{
		CompletionMode: config.WorkflowCompletionModeShellCommand,
		RuntimeClientFactory: runtimewire.RuntimeClientFactoryFunc(func(ctx context.Context, request runtimewire.RuntimeClientRequest) (llm.Client, error) {
			if request.Purpose == runtimewire.RuntimeClientPurposeWorkflow {
				select {
				case <-blockResumePreparation:
					resumePreparationOnce.Do(func() { close(resumePreparationEntered) })
					select {
					case <-releaseResumePreparation:
					case <-ctx.Done():
		...
		Project: serverapi.ProjectCreateRequest{
			DisplayName: "Terminal interrupt resume",
			ProjectKey:  "TERMRES",
		},
		CreateWorkflow: func(t *testing.T, ctx context.Context, store *workflowstore.Store) workflow.WorkflowID {
	...
	persistedWorkflowID := string(fixture.workflowID)
	task, err := fixture.appCore.WorkflowClient().CreateWorkflowTask(fixture.ctx, serverapi.WorkflowTaskCreateRequest{
		ProjectID:         fixture.projectID,
		WorkflowID:        &persistedWorkflowID,
		Title:             "Resume after terminal interrupt",
		SourceWorkspaceID: fixture.workspaceID,
	})
	if err != nil {
		t.Fatalf("CreateWorkflowTask: %v", err)
	...
	testsetup.RequireUntil(t, time.Now().Add(5*time.Second), 20*time.Millisecond, func() bool {
		detail, err := fixture.appCore.WorkflowClient().GetWorkflowTask(fixture.ctx, serverapi.WorkflowTaskGetRequest{TaskID: task.Task.ID})
		if err != nil {
			t.Fatalf("GetWorkflowTask active: %v", err)
		}
		if len(detail.Task.CurrentSessionIDs) != 1 {
			return false
		}
		sessionID = detail.Task.CurrentSessionIDs[0]
		return detail.Task.Actions.CanInterrupt
	}, "workflow task never exposed its live session")

	attachmentResponse, err := fixture.appCore.SessionRuntimeClient().ActivateSessionRuntime(fixture.ctx, serverapi.SessionRuntimeActivateRequest{
		ClientRequestID: "activate-terminal-runtime",
		SessionID:       sessionID,
		OwnerID:         "terminal-test-owner",
		ActiveSettings:  fixture.appCore.Config().Settings,
		Source:          fixture.appCore.Config().Source,
	})
	if err != nil {
		t.Fatalf("ActivateSessionRuntime: %v", err)
	}
	t.Cleanup(func() {
		_, _ = fixture.appCore.SessionRuntimeClient().ReleaseSessionRuntime(context.Background(), serverapi.SessionRuntimeReleaseRequest{
			ClientRequestID: "release-terminal-runtime",
			Attachment:      attachmentResponse.Attachment,
			OwnerID:         "terminal-test-owner",
		})
	})

	if _, err := fixture.appCore.RuntimeControlClient().Interrupt(fixture.ctx, serverapi.RuntimeInterruptRequest{
		ClientRequestID: "interrupt-terminal-runtime",
		SessionID:       sessionID,
	}); err != nil {
	...
	testsetup.RequireUntil(t, time.Now().Add(5*time.Second), 20*time.Millisecond, func() bool {
		detail, err := fixture.appCore.WorkflowClient().GetWorkflowTask(fixture.ctx, serverapi.WorkflowTaskGetRequest{TaskID: task.Task.ID})
		if err != nil {
			t.Fatalf("GetWorkflowTask interrupted: %v", err)
		}
		return detail.Task.Actions.CanResume && !detail.Task.Actions.CanInterrupt
	}, "terminal interrupt never made the workflow resumable")

	submitID := runtimeids.NewRuntimeClientRequestID()
	if _, err := fixture.appCore.RuntimeControlClient().SubmitUserTurn(fixture.ctx, serverapi.RuntimeSubmitUserTurnRequest{
		...
		Text: "finish this terminal turn",
	}); err != nil {
		t.Fatalf("terminal SubmitUserTurn: %v", err)
	}

	close(blockResumePreparation)
	resumeResult := make(chan error, 1)
	go func() {
		_, resumeErr := fixture.appCore.WorkflowClient().ResumeWorkflowTask(
			fixture.ctx,
			serverapi.WorkflowTaskResumeRequest{TaskID: task.Task.ID},
		)
		resumeResult <- resumeErr
	}()
	select {
	case <-resumePreparationEntered:
	case <-time.After(5 * time.Second):
		t.Fatal("ResumeWorkflowTask did not reach runtime preparation")
	}

	duringPreparation, err := fixture.appCore.WorkflowClient().GetWorkflowTask(
		fixture.ctx,
		serverapi.WorkflowTaskGetRequest{TaskID: task.Task.ID},
	)
	if err != nil {
		t.Fatalf("GetWorkflowTask during Resume preparation: %v", err)
	}
	if !duringPreparation.Task.Actions.CanResume ||
		duringPreparation.Task.Actions.CanInterrupt ||
		len(duringPreparation.Task.CurrentSessionIDs) != 1 {
		t.Fatalf(
			"task during blocked Resume preparation = status=%+v actions=%+v sessions=%v, want interrupted and resumable until a new Exact Execution Scope exists",
			duringPreparation.Task.Status,
			duringPreparation.Task.Actions,
			duringPreparation.Task.CurrentSessionIDs,
		)
	}
	...
	case resumeErr := <-resumeResult:
		if resumeErr != nil {
			t.Fatalf("ResumeWorkflowTask after terminal final answer: %v", resumeErr)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("ResumeWorkflowTask did not return after runtime preparation completed")
	}
	testsetup.RequireUntil(t, time.Now().Add(5*time.Second), 20*time.Millisecond, func() bool {
		detail, detailErr := fixture.appCore.WorkflowClient().GetWorkflowTask(
			fixture.ctx,
			serverapi.WorkflowTaskGetRequest{TaskID: task.Task.ID},
		)
		if detailErr != nil {
			t.Fatalf("GetWorkflowTask resumed: %v", detailErr)
		}
		return detail.Task.Actions.CanInterrupt &&
			!detail.Task.Actions.CanResume &&
			len(detail.Task.CurrentSessionIDs) == 1
	}, "Resume returned without establishing a new Exact Execution Scope")
}
...
func newComposedWorkflowFixture(t *testing.T, options composedWorkflowFixtureOptions) composedWorkflowFixture {
	...
	if completionMode == "" {
		completionMode = config.WorkflowCompletionModeStructuredOutput
	}
	resolved.Config.Settings.Workflow.CompletionMode = completionMode
	authSupport, err := serverbootstrap.BuildAuthSupport(auth.NewMemoryStore(auth.EmptyState()), nil, nil)
	if err != nil {
		t.Fatalf("BuildAuthSupport: %v", err)
	}
