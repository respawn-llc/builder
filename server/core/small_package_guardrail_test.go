...
var allowedSmallPackages = map[string]string{
	...
	"server/metadata/sqliteextensions":            "sole process-wide registrar for metadata SQLite collations and deterministic Task-search functions, which must run before every database connection opens",
	...
	"server/runtimecommand":                       "goal-command authority and its one approved exact-execution adapter keep live/dormant selection out of runtimecontrol without a package-wide execution gateway",
	...
	"shared/tasksearchtext/normalizationgen":      "repo-owned generator command that proves and materializes the pinned SQLite FTS5 normalization contract without a runtime dependency on translated SQLite internals",
