...
var allowedSmallPackages = map[string]string{
	...
	"shared/boundedio":                            "scope-neutral bounded process-output storage shared by shell postprocessing, workflow scripts, worktree setup, and lifecycle hook diagnostics without coupling those domain policies",
	"shared/lifecyclecontract":                    "scope-neutral external lifecycle event contract and bounded schema encoder kept below TUI adapters and reserved future server-hook adapters",
	...
	"shared/ownedprocess":                         "cross-platform joined process-tree ownership shared by workflow scripts and lifecycle hook dispatch without importing either domain's command or JSON policy",
