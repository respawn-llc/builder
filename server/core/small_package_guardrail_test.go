...
var allowedSmallPackages = map[string]string{
	...
	"server/mutationlane":                         "shared keyed in-process mutation serialization primitive used by project, workflow, and worktree owners so those owners do not duplicate lock registries",
	...
	"server/workflowvalidation":                   "canonical workflow definition validation seam shared by store, service, and view without coupling the workflow domain to API projection",
