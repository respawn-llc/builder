...
var allowedSmallPackages = map[string]string{
	...
	"server/workflow/label":                       "workflow-owned label identity, name validation, and Unicode comparison domain module shared by persistence and generation without coupling the workflow root package to desktop tooling",
	...
	"shared/labelcontract":                        "minimal label bounds contract required below both the workflow domain and shared server API without introducing a server-to-API dependency",
