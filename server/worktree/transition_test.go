...
func TestInterruptedModelStepDoesNotRetargetAfterWorkspaceLockWait(t *testing.T) {
	...
	if !errors.As(err, &immediate) || immediate.Kind != serverapi.WorktreeImmediateTransitionOriginInactive ||
		ack != (serverapi.WorktreeScheduledAcknowledgement{}) || target.Worktree != nil ||
		topologyErr != nil || matchErr != nil || match.entry.Variant != serverapi.WorktreeTopologyVariantExternal {
		t.Fatalf("ack=%+v err=%v target=%+v topology=%+v/%v/%v", ack, err, target, match.entry.Variant, topologyErr, matchErr)
	}
}

func TestInterruptedModelStepRejectsCurrentWorktreeEnter(t *testing.T) {
	env := newServiceTestEnv(t)
	gate, started := make(chan struct{}), make(chan struct{}, 1)
	var active atomic.Bool
	active.Store(true)
...
func enterModelWorktree(env *serviceTestEnv, selector string) (serverapi.WorktreeScheduledAcknowledgement, error) {
	return env.service.EnterWorktree(env.ctx, serverapi.WorktreeEnterRequest{
		OperationID: serverapi.NewWorktreeOperationID(),
		SessionID:   env.session.Meta().SessionID,
		Selector:    selector,
