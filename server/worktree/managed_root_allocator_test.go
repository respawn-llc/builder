...
func TestManagedRootAllocatorReplacementRetriesReservedRootCollision(t *testing.T) {
	base := filepath.Join(t.TempDir(), "worktrees")
	workspace := filepath.Join(t.TempDir(), "Builder CLI")
	if err := os.MkdirAll(workspace, 0o755); err != nil {
		t.Fatalf("create workspace: %v", err)
	...
	if err := os.Mkdir(filepath.Join(parent, "KENT-453-111"), 0o755); err != nil {
		t.Fatalf("reserve colliding root: %v", err)
	}
	location, err := allocator.reserveTaskLocation(
		workspace,
		"KENT-453",
	...
	if err != nil {
		t.Fatalf("reserve replacement location: %v", err)
	}
	if location.BranchName != "KENT-453-2222" || filepath.Base(location.Root) != location.BranchName {
		t.Fatalf("replacement location = %+v, want shared KENT-453-2222 suffix", location)
	}
}

func TestManagedRootAllocatorReplacementUsesOneRootAndBranchSuffixAfterBranchCollision(t *testing.T) {
	base := filepath.Join(t.TempDir(), "worktrees")
	workspace := filepath.Join(t.TempDir(), "Builder CLI")
	if err := os.MkdirAll(workspace, 0o755); err != nil {
		t.Fatalf("create workspace: %v", err)
	...
	location, err := allocator.reserveTaskLocation(
		...
		func(branch string) (bool, error) {
			return branch == "KENT-453-111", nil
		},
	)
	if err != nil {
		t.Fatalf("reserve replacement location: %v", err)
	...
	parent := filepath.Dir(location.Root)
	if _, err := os.Stat(filepath.Join(parent, "KENT-453-111")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("branch-collision reservation was not released: %v", err)
	}
}

func TestManagedRootAllocatorReplacementPanicsAfterCollisionsAreExhausted(t *testing.T) {
	base := filepath.Join(t.TempDir(), "worktrees")
	workspace := filepath.Join(t.TempDir(), "Builder CLI")
	if err := os.MkdirAll(workspace, 0o755); err != nil {
		t.Fatalf("create workspace: %v", err)
	...
	defer func() {
		...
		if !ok ||
			exhaustion.Operation != "replacement-task-location" ||
			exhaustion.TaskShortID == nil ||
			*exhaustion.TaskShortID != "KENT-453" {
			t.Fatalf("panic = %#v, want typed replacement Task exhaustion", value)
		}
	}()
	_, _ = allocator.reserveTaskLocation(
		workspace,
		"KENT-453",
		managedTaskWorktreeLocationReplacement,
