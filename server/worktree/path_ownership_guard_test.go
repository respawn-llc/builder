...
func typedPathOwnershipViolations(pkg *packages.Package) []string {
	...
	for _, file := range pkg.Syntax {
		...
		ast.Inspect(file, func(node ast.Node) bool {
			...
			case *ast.CallExpr:
				if typedForbiddenPathCall(pkg, current, identities) {
					violations = append(violations, "typed Workspace identity path construction")
				}
			case *ast.BinaryExpr:
				if current.Op == token.ADD && typedContainsWorkspaceIdentity(pkg, current, identities, workspaceType) {
...
func typedContainsWorkspaceIdentity(pkg *packages.Package, expression ast.Expr, objects map[types.Object]bool, workspaceType *types.Named) bool {
	...
	ast.Inspect(expression, func(node ast.Node) bool {
		...
		case *ast.SelectorExpr:
			...
			if selection != nil && sameNamedType(selection.Recv(), workspaceType) && selection.Obj().Name() == "ID" {
				found = true
			}
		}
		return !found
...
func typedForbiddenPathCall(pkg *packages.Package, call *ast.CallExpr, objects map[types.Object]bool) bool {
