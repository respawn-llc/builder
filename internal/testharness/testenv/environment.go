...
// ClearAtProcessStart clears environmentName before package tests run.
func ClearAtProcessStart(environmentName string) {
	if err := os.Unsetenv(environmentName); err != nil {
		panic(fmt.Sprintf("clear test process environment %s: %v", environmentName, err))
	}
}

// AssertUnsetAtProcessStart verifies that package initialization clears
// environmentName before the selected test starts in a child process.
func AssertUnsetAtProcessStart(
	t testing.TB,
	rootTestName string,
