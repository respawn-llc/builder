...
#[test]
fn lint_policy_rejects_desktop_process_execution_adapter() {
    let repo = repo();
    write(
        &repo.path().join("apps/desktop/src-tauri/Cargo.toml"),
        r#"
[package]
name = "app-desktop"
version = "0.0.0"
...
"#,
    ...
    write(
        &repo.path().join("apps/desktop/src-tauri/src/lib.rs"),
        "#![forbid(unsafe_code)]\nuse std::process::Command;\npub fn run() { let _ = Command::new(\"hook\"); }\n",
    );

    let findings = manifest_check::lint_policy::check(repo.path()).unwrap_err();

    assert!(findings.iter().any(|finding| {
        finding.code == "desktop_lifecycle_executable_adapter"
            && finding.path.as_deref()
                == Some(Path::new("apps/desktop/src-tauri/src/lib.rs"))
    }));
}

#[test]
fn lint_policy_rejects_desktop_process_execution_dependency() {
    let repo = repo();
    write(
        &repo.path().join("apps/desktop/src-tauri/Cargo.toml"),
        r#"
...
"#,
    ...
    assert!(findings.iter().any(|finding| {
        finding.code == "desktop_lifecycle_executable_adapter"
            && finding.path.as_deref() == Some(Path::new("apps/desktop/src-tauri/Cargo.toml"))
    }));
}

#[test]
fn lint_policy_rejects_renamed_desktop_process_execution_dependency() {
    let repo = repo();
    write(
        &repo.path().join("apps/desktop/src-tauri/Cargo.toml"),
        r#"
...
[dependencies]
desktop-shell = { package = "tauri-plugin-shell", version = "2" }
"#,
    );
    write(
        &repo.path().join("apps/desktop/src-tauri/src/lib.rs"),
    ...
    assert!(findings
        .iter()
        .any(|finding| finding.code == "desktop_lifecycle_executable_adapter"));
}

#[test]
fn lint_policy_rejects_desktop_async_process_feature() {
    let repo = repo();
    write(
        &repo.path().join("apps/desktop/src-tauri/Cargo.toml"),
        r#"
