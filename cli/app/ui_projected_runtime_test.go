...
func TestHydratingClientAndLiveClientConvergeWithoutDuplicateCommittedRows(t *testing.T) {
	...
	hydratingCommitted := stripANSIAndTrimRight(committedOngoingProjectionForTest(hydrating.view).Render(tui.TranscriptDivider))
	liveCommitted := stripANSIAndTrimRight(committedOngoingProjectionForTest(live.view).Render(tui.TranscriptDivider))
