...
func TestRuntimeAttachmentRewireKeepsOwnerAndGenerationUntilFinalClose(t *testing.T) {
	activateCalls := 0
	releaseCalls := 0
	var ownerID string
	server := runtimeAttachmentTestServer{
		runtime: &recordingSessionRuntimeClient{
			activate: func(_ context.Context, req serverapi.SessionRuntimeActivateRequest) (serverapi.SessionRuntimeActivateResponse, error) {
				...
				if ownerID == "" {
					ownerID = req.OwnerID
				} else if req.OwnerID != ownerID {
					t.Fatalf("Runtime owner changed from %q to %q", ownerID, req.OwnerID)
				}
				return sessionRuntimeActivateResponse(req.SessionID, 1), nil
			},
			release: func(_ context.Context, req serverapi.SessionRuntimeReleaseRequest) (serverapi.SessionRuntimeReleaseResponse, error) {
				releaseCalls++
				if req.OwnerID != ownerID || req.Attachment.Generation != 1 {
					t.Fatalf("final release = %+v, want owner %q generation 1", req, ownerID)
				}
				return serverapi.SessionRuntimeReleaseResponse{}, nil
			},
		},
	...
	plan, err := prepareSharedRuntime(context.Background(), server, sessionLaunchPlan{
		SessionID:      "session-transfer",
		ActiveSettings: config.Settings{Model: "source-model"},
	}, io.Discard, "test")
	if err != nil {
		t.Fatalf("prepareSharedRuntime: %v", err)
	}
	...
	if activateCalls != 1 {
		t.Fatalf("activation calls after handoff = %d, want 1", activateCalls)
	}
	if err := plan.Close(); err != nil {
		t.Fatalf("Close: %v", err)
	}
	if releaseCalls != 1 {
		t.Fatalf("final release calls = %d, want 1", releaseCalls)
	}
}
