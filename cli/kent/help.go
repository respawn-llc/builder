...
var (
	...
	taskResumeUsage         = leafCommandUsage(config.Command+" task resume <task> [--project <project>] [--execution-target none|head|default-branch|ref:<revision>] [--json]", "Resume interrupted work on a task.", "", "A workflow Session may resume another Task, but not its own.")
	...
	taskMoveUsage           = leafCommandUsage(config.Command+" task move <task> <target-node-id> [--project <project>] [--commentary <text>] [--output name=value] [--execution-target none|head|default-branch|ref:<revision>] [--ignore-dependencies] [--json]", "Move a task to a workflow node.", "", "A workflow Session may move another Task, but not its own.")
