...
const ids = ["123e4567-e89b-42d3-a456-426614174000", "223e4567-e89b-42d3-a456-426614174000",
  "323e4567-e89b-42d3-a456-426614174000", "423e4567-e89b-42d3-a456-426614174000"] as const;
...
const items = [
  ...
  { ...base(ids[2], "worktree_transition", "/wt switch feature"), worktree_transition: { transition: "enter", selector: "feature" } },
...
describe("Desktop Pending Work client", () => {
  it("validates representative closed contracts and supplied order", () => {
    ...
    expect(decoded.items.map((item) => item.canonicalInput)).toEqual(items.map((item) => item.canonical_input));
    expect(decoded.items[2]?.id.toJSONValue()).toBe(parseWorktreeOperationID(ids[2]).toJSONValue());
    const invalid = [
      ...items.map((item) => ({ items: [{ ...item, canonical_input: "wrong" }] })),
      { items: [{ ...message, manual_compaction: {} }] },
      { items: [{ ...compact, message: { text: "wrong family" } }] },
      { items: [{ ...leave, worktree_transition: { transition: "leave", selector: "forbidden" } }] },
      { items: [compact, message] },
      { items: [{ ...message, id: "123e4567-e89b-12d3-a456-426614174000" }] },
    ];
    ...
    expect(pendingWorkChangedEventSchema.safeParse({ PendingWork: {} }).success).toBe(false);
    const restoration = { Restoration: { item_id: ids[1], kind: "manual_compaction", canonical_input: "/compact" } };
    expect(pendingWorkTechnicalRestorationEventSchema.parse(restoration).restoration.itemID.toJSONValue()).toBe(ids[1]);
    const feedback = { Kind: "fast_mode", Changed: true, SessionName: null, Thinking: null,
      FastMode: true, Supervisor: null, Questions: null, AutoCompaction: null } as const;
  ...
  it("uses compact, list, and remove routes with typed identities", async () => {
    const transport = new FakeRpcTransport([
      ...
      { method: "runtime.pendingWork.remove", result: {
    ...
    expect(await client.removePendingWork("session-1", item.id)).toEqual({
      kind: "worktree_transition", canonicalInput: "/wt switch feature",
    });
    expect(transport.dedicatedCalls[0]).toMatchObject({ method: "runtime.compactContext",
      params: { session_id: "session-1", request_id: ids[1], admission: { guidance: "keep decisions" } } });
    expect(transport.calls.map((call) => call.method)).toEqual([
      "runtime.pendingWork.list", "runtime.pendingWork.remove",
    ]);
  ...
  it("decodes direct and nested typed failures", () => {
    const failures = [
      ...
      [rpcErrorCodes.manualCompactionTooSoon, { reason: "too_soon" }, { kind: "manual_compaction", reason: "too_soon" }],
      [rpcErrorCodes.manualCompactionDisabled, { reason: "disabled" }, { kind: "manual_compaction", reason: "disabled" }],
      [rpcErrorCodes.manualCompactionActive, { reason: "active" }, { kind: "manual_compaction", reason: "active" }],
    ] as const;
    ...
    const notPending = decodePendingWorkError(rpcError("runtime.pendingWork.remove",
      rpcErrorCodes.pendingWorkNotPending, { item_id: ids[0] }))?.detail;
    expect(notPending?.kind).toBe("not_pending");
    if (notPending?.kind === "not_pending") expect(notPending.itemID.toJSONValue()).toBe(ids[0]);
    expect(decodePendingWorkError(rpcError("runtime.compactContext",
      rpcErrorCodes.pendingWorkCapacity, { reason: "wrong" }))).toBeNull();
  });
});
