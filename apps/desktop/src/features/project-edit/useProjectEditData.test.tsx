...
const changed = vi.hoisted(
  (): { handler: ((event: { projectID: string }) => void) | undefined } => ({ handler: undefined }),
);
...
const wrapper = (client: QueryClient) =>
  ({ children }: Readonly<{ children: ReactNode }>) => <QueryClientProvider client={client}>{children}</QueryClientProvider>;
...
describe("Project Settings Workspace refresh ownership", () => {
  ...
  it("native Workspace changes leave the canonical catalog untouched", async () => {
    ...
    renderHook(() => { useProjectWorkspaceChangedEvents(nativeBridge, "project-1"); }, {
