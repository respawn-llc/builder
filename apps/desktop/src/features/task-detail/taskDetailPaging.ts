...
export const selectedFeed = <Comments, Activity>(tab: DetailTab, comments: Comments, activity: Activity): Comments | Activity => tab === "comments" ? comments : activity;
export const feedPixelOffsetRequest = (attentionPending: boolean, feedPending: boolean, request: VirtualizedPixelOffsetRequest | undefined) => attentionPending || feedPending ? undefined : request;
type TaskDetailPagingInput = Readonly<{ activity: ReturnType<typeof useTaskActivity>; comments: ReturnType<typeof useTaskComments>; detailID: string; selectedTab: DetailTab }>;
export function taskDetailPaging({ activity, comments, detailID, selectedTab }: TaskDetailPagingInput) {
  ...
  const loadKey = (direction: "previous" | "next", offset: number | null | undefined) => offset === undefined || offset === null ? undefined : `${detailID}:${selectedTab}:${direction}:${offset.toString()}:${data.dataUpdatedAt.toString()}`;
  return {
    error: data.error,
