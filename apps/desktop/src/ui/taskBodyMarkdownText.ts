...
function visitText(value: string, position: MarkdownNode["position"], source: string, projection: Projection): void {
  ...
  if (start === undefined || end === undefined) { emit(value, projection); return; }
  const original = source.slice(start, end);
  feed(original, projection, original === value); if (original !== value && projection.html === undefined) emit(value, projection);
}
function feed(value: string, projection: Projection, visible: boolean): void {
  for (let index = 0; index < value.length; ) {
    ...
    if (state === undefined) {
      ...
      if (html === undefined) { if (visible) emitCharacter(value.charAt(index), projection); index += 1; continue; }
      space(projection); projection.html = html.state; index = html.index; continue;
    }
    const next = htmlAdvance(value, index, state);
    projection.html = next.state; index = next.index;
  }
}
function htmlStart(value: string, start: number): { index: number; state: HtmlState } | undefined {
  ...
  return { index, state: closing ? { kind: "tag" } : { kind: "tag", name: value.slice(nameStart, index).toLowerCase() } };
}
...
function emit(value: string | null | undefined, projection: Projection): void {
  if (value !== null && value !== undefined) for (const character of value) emitCharacter(character, projection);
}
function emitCharacter(character: string, projection: Projection): void {
  if (whitespace.has(character)) { space(projection); return; }
...
function hiddenRawText(name: string): boolean { return name === "script" || name === "style" || name === "template"; }
function isNameChar(value: string): boolean { const code = value.charCodeAt(0); return (code >= 48 && code <= 57) || (code >= 65 && code <= 90) || (code >= 97 && code <= 122) || value === "-"; }
