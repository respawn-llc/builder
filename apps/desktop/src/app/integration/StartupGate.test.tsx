...
import { ServerRootMismatchError, StartupConfigurationError, TransportError } from "@/api";
import {
  clientProtocolVersion as protocolVersion,
  createTestServices,
  startupRoutes,
...
describe("StartupGate", () => {
  ...
  it("rechecks readiness when the setup guide check-again button is used", async () => {
    ...
    await waitFor(() => {
      expect(
        services.transport.calls.filter((call) => call.method === "server.readiness.get").length,
